...
void Sparse_net_optimizer::step(
  ...
  for(vector<sdouble32>& input_sample : input_samples){ /* For every provided sample */
    calculate_gradient(input_sample, sample_iterator, 0);
    ++sample_iterator;
...
void Sparse_net_optimizer::calculate_gradient(
  ...
  feature_buffers[solve_thread_index] = solver.solve(input_sample);
  ...
  for(sint32 row_iterator = 0; row_iterator < gradient_step.cols_size(); ++row_iterator){
    ...
    while(process_thread_iterator < gradient_step.cols(row_iterator)){
      ...
      while(
        ...
        calculate_threads.push_back(thread(
          &Sparse_net_optimizer::propagate_errors_back, this, solve_thread_index,
  ...
  while(static_cast<int>(process_thread_iterator) < net.neuron_array_size()){
    while(
      ...
      calculate_threads.push_back(thread(
        ...
        process_thread_iterator, std::ref(input_sample), solve_thread_index
      ));
...
void Sparse_net_optimizer::calculate_weight_gradients(
  ...
  new_value = (buffer + (*error_values[solve_thread_index][neuron_index])) * step_size;
  while(!weight_gradients[solve_thread_index][index]->compare_exchange_weak(buffer, new_value)){
    buffer = *weight_gradients[solve_thread_index][index];
    new_value = (buffer + (*error_values[solve_thread_index][neuron_index])) * step_size;
  }
  ...
  neuron_router.run_for_neuron_inputs(neuron_index,[&](sint32 child_index){
    ...
    new_value = buffer 
      + (neuron_input * *error_values[solve_thread_index][neuron_index]) * step_size;
