...
Solution test_solution_builder_manually(google::protobuf::Arena* arena, sdouble32 device_max_megabytes){
  ...
  SparseNet net = (
    *Sparse_net_builder().input_size(50).expected_input_range(5.0)
    .output_neuron_number(2).arena_ptr(arena)
    .cost_function(COST_FUNCTION_QUADRATIC).dense_layers(net_structure)
  );

  Solution solution = ( /* Generate solution from Net */
    *Solution_builder().max_solve_threads(4).device_max_megabytes(device_max_megabytes).arena_ptr(arena).build(net)
  );
