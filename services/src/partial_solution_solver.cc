...
vector<sdouble32> Partial_solution_solver::solve(const Partial_solution& detail, const vector<sdouble32>& input_data){
  if(
    ...
    Synapse_iterator iter(detail.inside_index_starts(),detail.inside_index_sizes());
    data.clear();
    ...
    for(
      ...
      iter.iterate_unsafe([&](int synapse_index){
  ...
  }else throw "Provided Partial Solution is inconsistent!";
}
