...
/**
 * @brief      This class helps query a state paired with a number of Action
 * Q-value pairs with only const access
 */
class RAFKO_EXPORT RafQSetItemConstView {
  /**
   * @brief     Calculates the offsets for the q-values in the common buffer
   * based on the given action size and count
   */
  static std::vector<std::uint32_t> q_value_offset(std::uint32_t action_size,
                                                   std::uint32_t action_count) {
    std::vector<std::uint32_t> offsets;
    for (std::uint32_t action_index = 0u; action_index < action_count;
         ++action_index)
  ...
  /**
   * @brief     Calculates the offsets for the actions in the common buffer
   * based on the given action size and count
   */
  static std::vector<std::uint32_t> action_offset(std::uint32_t action_size,
                                                  std::uint32_t action_count) {
    std::vector<std::uint32_t> offsets;
    for (std::uint32_t action_index = 0u; action_index < action_count;
         ++action_index)
...
public:
  ...
  RafQSetItemConstView(const FeatureVector &state, const FeatureVector &actions,
                       std::uint32_t action_size,
                       std::uint32_t action_count = 1)
      : m_actionCount(action_count), m_state(state), m_actions(actions),
        m_stateSize(state.size()), m_actionSize(action_size),
   ...
   *
   * @return    Const iterator to the first element of the action under the
   * given index
   */
   ...
   */
  const FeatureVector &state() const { return m_state; }
   ...
   *
   * @param[in]     action_index    The index of the action to query(q-values
   * are paired with actions)
   *
  ...
  double avg_q_value() const {
    double count = 0.0;
    double sum = 0.0;
    for (std::uint32_t action_index = 0; action_index < m_actionCount;
         ++action_index) {
      if ((m_actionSize > std::count((*this)[action_index],
                                     (*this)[action_index] + m_actionSize,
                                     0.0)) &&
  ...
  double max_q_value() const {
    ...
    for (std::uint32_t action_index = 1; action_index < m_actionCount;
         ++action_index) {
   ...
   */
  std::uint32_t action_count() { return m_actionCount; }
  ...
  /**
   * @brief     Provides the number of elements stored in the enclosed actions
   * buffer
   *
   * @return    The number of elements making up all of the enclosed actions and
   * q-values
   */
   ...
   *
   * @param[in]     action      Const reference to the buffer containing the
   * actions
   * @param         q_value     The q-value to assign the action buffer
   */
  static FeatureVector action_slot(const FeatureVector &action,
                                   double q_value) {
   ...
   *            Buffer is considered to contain at least one action of the given
   * size
   *
   ...
   */
  static FeatureView best_action_slot(FeatureView actions_buffer,
                                      std::uint32_t action_size) {
    RFASSERT(action_slot_size(action_size) <= actions_buffer.size());
    return {actions_buffer.begin() + 1,
            actions_buffer.begin() + 1 + action_size};
  }
...
/**
 * @brief      This class helps read and update a state paired with a number of
 * Action Q-value pairs
 */
...
public:
   ...
   * @param     action_index    The index of the action to update the q-value
   * for
   */
  ...
  void swap_action(std::uint32_t source, std::uint32_t target) {
    ...
    if (source == target)
      return;
    std::swap_ranges((*this)[source], (*this)[source] + m_actionSize,
                     (*this)[target]);
   ...
   * @param     source_action_index     The index of the action in @xp_element
   * to copy the data from
   * @param     target_action_index     The index of the action in the current
   * object to copy the data to
   */
  void take_over(RafQSetItemConstView xp_element,
                 std::uint32_t source_action_index = 0u,
                 std::uint32_t target_action_index = 0u) {
    RFASSERT(source_action_index < xp_element.action_count());
    RFASSERT(target_action_index < m_actionCount);
    std::copy(xp_element[source_action_index],
              xp_element[source_action_index] + m_actionSize,
              (*this)[target_action_index]);
  }

  /**
   * @brief     Provides access to the buffer of the action under the given
   * action index. Indexes are checked for OOB
   *
   * @param     actions_buffer      Rreference to the buffer containing the
   * action-q-value pairs
   * @param     action_size         Size of one action inside the buffer
   ...
   */
  static FeatureVector::iterator
  action_iterator(FeatureVector &actions_buffer, std::uint32_t action_size,
                  std::uint32_t action_index = 0) {
    RFASSERT((((action_size + 1) * action_index) + action_size + 1) <=
             actions_buffer.size());
  ...
  /**
   * @brief     Provides access to the buffer of the q-value under the given
   * action index. Indexes are checked for OOB
   *
   * @param     actions_buffer      Rreference to the buffer containing the
   * action-q-value pairs
   ...
   */
  static FeatureVector::iterator
  q_value_iterator(FeatureVector &actions_buffer, std::uint32_t action_size,
                   std::uint32_t action_index = 0) {
    RFASSERT((((action_size + 1) * action_index) + action_size + 1) <=
             actions_buffer.size());
    /* Action slot start + (slot size *  slot_index )         + q_value offset
...
/**
 * @brief      This class stores and provides a set of states and connected
 * actions with corresponding QValues
 */
class RAFKO_EXPORT RafQSet : public RafkoDataSet {
public:
  using FeatureVector = RafQEnvironment::FeatureVector;
  using FeatureView = RafQEnvironment::FeatureView;
  using MaybeFeatureVector =
      std::optional<std::reference_wrapper<const FeatureVector>>;
  using AnyData = RafQEnvironment::AnyData;

  RafQSet(const rafko_mainframe::RafkoSettings &settings,
  ...
  /**
   * @brief     Export every item in the set into the @DataSetPackage message
   * for later use
   *
   * @return    A @DataSetPackage containing all of the stored items in the set
   * with a sequence size of 1
   */
  ...
  /**
   * @brief     Export the best action from the set into a @DataSetPackage for
   * later use. Produces an empty package if long enough sequences could not be
   * generated
   *
   * @param     preferred_sequence_size     The number of items expected to be
   * in one sequence
   *
   * @return    A @DataSetPackage containing all of the best actions in temporal
   * order, or an empty package
   */
  DataSetPackage
  generate_best_sequences(std::uint32_t preferred_sequence_size) const;

  /**
   * @brief     Provides a View matching the given state View should there be a
   * match inside the set.
   *
   * @param[in]       state                   The data to look for in the stored
   * states
   * @param[inout]    result_index_buffer     A pointer to an integer to store
   * the result index should there be any
   *
   * @return    An optional, storing the reference to the stored state buffer
   * matching the given one, if there is any
   */
  MaybeFeatureVector
  look_up(FeatureView state,
          std::uint32_t *result_index_buffer = nullptr) const;

  /**
   * @brief     Updates the q-set with the given state-action-q-value pairs. The
   * sizes of the given vectors must match exactly.
   *
   * @param[in]     state_buffer          A vector of states to look for
   * @param[in]     actions_buffer        A vector of action-q-value pairs to
   * update the data with
   * @param[in]     user_data_buffer      Custom data for each state, to help
   * environment restore state from hidden data
   * @param[in]     progress_callback     A function to call at each progress
   * update with a value of 0..1 representing the progress of the operation
   */
  void incorporate(
  ...
  /**
   * @brief     Erases elements from the set if it gets greater, than the given
   * size. Elements with the smallest q-values are erased
   *
  ...
  /**
   * @brief     Provides const access to an item in the set under the given
   * index
   *
   * @param     index     The index of the item to query
   *
   * @return  An instance of @RafQSetItemConstView for the elements under the
   * given index
   */
  RafQSetItemConstView operator[](std::uint32_t index) const {
    RFASSERT(index < get_number_of_sequences());
    return RafQSetItemConstView(m_statesBuffer[index], m_actionsBuffer[index],
   ...
   * @param     index     The index of the item to query
   *
   * @return  An instance of @RafQSetItemView for the elements under the given
   * index
   */
  RafQSetItemView operator[](std::uint32_t index) {
    RFASSERT(index < get_number_of_sequences());
    return RafQSetItemView(m_statesBuffer[index], m_actionsBuffer[index],
                           m_environment.action_size(), m_actionCount);
  }

  /**
   * @brief     Provides the number of action assigned for one state by the
   * q-set policy
   *
   * @return    Number of action-q-value pairs assigned to one state inside the
   * q-set
   */
  constexpr std::uint32_t action_count() { return m_actionCount; }

  /**
   * @brief     Gives back the maximum number of elements the q-set is
   * configured to contain
   *
   * @return    Number of elements the set is configured to keep when extending
   */
  constexpr std::uint32_t max_size() const { return m_maxSetSize; }
  ...
  std::uint32_t get_feature_size() const override {
    return RafQSetItemView::feature_size(m_environment.action_size(),
                                         m_actionCount);
  }
  ...
  std::uint32_t get_sequence_size() const override { return 1u; }

  std::uint32_t get_prefill_inputs_number() const override { return 0u; }
...
private:
  ...
  /**
   * @brief     Calculates the Temporal difference value for the given
   * state-action-q-value pair
   *
   * @param         new_action_view       Object providing const acess to the
   * data for the given state-action-q-value pair
   * @param         old_q_value           The Q-value for the given object prior
   * to a recent update
   * @param[in]     user_data_buffer      Custom data for the new action state,
   * to help environment restore state from hidden data not included in the
   * state vector
   */
  double get_td_value(const RafQSetItemConstView &new_action_view,
                      double old_q_value, const AnyData &user_data = {}) const;
};
