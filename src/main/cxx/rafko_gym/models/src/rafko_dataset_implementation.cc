...
void RafkoDatasetImplementation::fill(const rafko_gym::DataSetPackage& samples){
  ...
  for(std::uint32_t raw_sample_iterator = 0; raw_sample_iterator < m_inputSamples.size(); ++ raw_sample_iterator){
    m_inputSamples[raw_sample_iterator] = std::vector<double>(samples.input_size());
    for(std::uint32_t input_iterator = 0; input_iterator < samples.input_size(); ++input_iterator)
      m_inputSamples[raw_sample_iterator][input_iterator] = samples.inputs(input_start_index + input_iterator);
    input_start_index += samples.input_size();
    if(raw_sample_iterator < m_labelSamples.size()){
      m_labelSamples[raw_sample_iterator] = std::vector<double>(samples.feature_size());
      for(std::uint32_t feature_iterator = 0; feature_iterator < samples.feature_size(); ++feature_iterator)
        m_labelSamples[raw_sample_iterator][feature_iterator] = samples.labels(feature_start_index + feature_iterator);
