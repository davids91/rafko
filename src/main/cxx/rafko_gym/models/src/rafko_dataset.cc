...
std::vector<cl::Event> RafkoDataSet::upload_inputs_to_buffer(
  ...
  std::uint32_t elements_in_a_sequence = get_sequence_size() + get_prefill_inputs_number();
  ...
  for(std::uint32_t raw_input_index = raw_input_start; raw_input_index < (raw_input_start + raw_input_num); ++raw_input_index){
    ...
    return_value = opencl_queue.enqueueWriteBuffer(
      ...
      get_input_sample(raw_input_index).data(), NULL, &events[raw_input_index - raw_input_start]
    );
    RFASSERT( return_value == CL_SUCCESS );
    input_buffer_byte_offset += (sizeof(double) * get_input_sample(raw_input_index).size());
  }
  return events;
}
...
std::vector<cl::Event> RafkoDataSet::upload_labels_to_buffer(
  ...
  RFASSERT( (start_index_inside_sequence + sequence_truncation) <= get_sequence_size() );
