...
/**
 * @brief      Error function handling and utilities for Binary Cross entropy
 * sum as described in
...
public:
  ...
  static std::string get_derivative_kernel_source(std::string label_value,
    ...
    return "- (" + label_value + " * " + feature_d + ") / (" + sample_number +
           " * (1.0 - " + feature_value + ")" + " * " + feature_value + ")";
  }
...
protected:
  ...
  constexpr double get_cell_error(double label_value,
    ...
    return (label_value *
            ...
            (((1.0) - label_value) *
             std::log((1.0) - std::min((0.9999999999999999), feature_value))));
  }
  ...
  constexpr double get_derivative(double label_value, double feature_value,
    ...
    return ((label_value - feature_value) * feature_d) /
           (sample_number * (1.0 - feature_value) * feature_value);
  }
  ...
  get_operation_kernel_source(std::string label_value,
    ...
    return ("( " + label_value + " * log(max(0.0000000000000001," +
            feature_value + ")) )" + std::string("+") + "( " +
            one_minus_label_value + " * log(1.0 - min(0.9999999999999999," +
            feature_value + ")) )");
  }
