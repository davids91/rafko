 ...
 *             The approximated gradients are collected into one gradient
 * fragment.
 */
class RAFKO_EXPORT RafkoNumericOptimizer
    : public rafko_mainframe::RafkoAutonomousEntity {
public:
  /**
   * @brief      Class Constructor
   *
   * @param      contexts                     An array of service contexts to
   * use for training the network
   * @param[in]  stochastic_evaluation_loops  Decideshow many stochastic
   * evaluations of the @neural_network shall count as one evaluation during
  ...
  RafkoNumericOptimizer &
  operator=(const RafkoNumericOptimizer &other) = delete; /* Copy assignment */
  RafkoNumericOptimizer &
  operator=(RafkoNumericOptimizer &&other) = delete; /* Move assignment */

  /**
   * @brief      Moves the network in a direction based on induvidual weight
   * gradients, approximates the gradients based on that and then reverts the
   * the weight change
   */
  void collect_approximates_from_weight_gradients();

  /**
   * @brief      Move the network in the given direction, collect approximate
   * gradient for it and then reverts the weight change
   *
   * @param      direction         The direction
   * @param[in]  save_to_fragment  Decides wether or not to add the results into
   * the collected gradient fragments
   */
  void convert_direction_to_gradient(std::vector<double> &direction,
                                     bool save_to_fragment);

  /**
   * @brief      Collects the approximate gradient of a single weight
   *
   * @param[in]  weight_index  The weight index to approximate for
   * @param      context       Reference to the context handling the evaluation
   * for the gradients
   *
  ...
  /**
   * @brief      Applies the colleted gradient fragment to the configured
   * network
   */
   ...
   */
  void add_to_fragment(std::uint32_t weight_index,
  ...
  /**
   * @brief   Function to set weight of the networks weight in bulk for
   * iterations; Filter vector must be equal in sze of the stored networks
   * weight table Meaning: no modification | 0 --> 1 | gradient * 1.0
   *
   ...
   *          Filter vector must be equal in sze of the stored networks weight
   * table Meaning: no modification | 0 --> 1 | gradient * 1.0
   *
  ...
  /**
   * @brief     Function to set weight of all of the the networks weight for
   * iterations; Meaning: no modification | 0 --> 1 | gradient * 1.0
   *
  ...
  /**
   * @brief   Function to set weight chance to be excluded in bulk for
   * iterations; Filter vector must be equal in sze of the stored networks
   * weight table Meaning: don't exclude weight: 0 --> 1 definitely exclude
   * weight
   *
   * @param[in]   filter    the filter to set for the whole weight array
   */
  ...
  /**
   * @brief   Function to set weight chance to be excluded in bulk for
   * iterations; The given value shall be set for all weights Meaning: don't
   * exclude weight: 0 --> 1 definitely exclude weight
   *
   * @param[in]   filter    the filter to set for the whole weight array
   */
  ...
  constexpr double get_error_estimation() const { return m_errorEstimation; }
  ...
  bool stop_training() {
    return (
        ...
        (((m_settings->get_training_strategy(
               Training_strategy::
                   training_strategy_stop_if_training_error_below_learning_rate) &&
...
private:
  ...
  /**
   * @brief      Evaluates the network in a stochastic manner the number of
   * configured times and return with the fittness/error value
   *
   * @return         The average of the resulting fitness values of the
   * evaluations
   */
  ...
  /**
   * @brief   Calculates the error value from a stochastic evaluation with the
   * network shifted to the given direction And does it in a thread-safe way for
   * the Original network
   *
   * @param   context                     The used context for stochastic
   * evaluation
   * @param   network_original_weights    The original weights of the network,
   * provided here as an optimiztion step
   * @param   direction                   The direction value to add to every
   * weight of the network
   */
  double
  get_error_from_direction(rafko_mainframe::RafkoContext &context,
   ...
   *             first adding it to the end, and then reverse iterating and
   * swapping elements until the desired position is reached
   *
   * @param      message_field  The message field
   * @param[in]  value          The value
   * @param[in]  position       The position
   */
  static void insert_element_at_position(
      google::protobuf::RepeatedField<double> &message_field, double value,
      std::uint32_t position) {
    *message_field.Add() = value;
    for (std::int32_t i(message_field.size() - 1);
         i > static_cast<std::int32_t>(position); --i)
