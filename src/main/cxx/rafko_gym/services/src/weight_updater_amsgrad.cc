...
void RafkoWeightUpdaterAMSGrad::iterate(const std::vector<double>& gradients){
  for(std::int32_t weight_index = 0; weight_index < m_network.weight_table_size(); ++weight_index){
    ...
    m_maxVariance[weight_index] = std::max(m_maxVariance[weight_index], (
...
double RafkoWeightUpdaterAMSGrad::get_new_velocity(std::uint32_t weight_index, const std::vector<double>& /*gradients*/) const{
    ...
    (m_settings.get_learning_rate() / (
      std::sqrt(
        m_maxVariance[weight_index] / (
          (1.0) - std::pow(
             m_settings.get_beta(), static_cast<double>(m_iterationCount)
         )
        )
      ...
      m_mean[weight_index] / (
        (1.0) - std::pow(
          m_settings.get_beta_2(), static_cast<double>(m_iterationCount)
        )
      )
    )
  );
