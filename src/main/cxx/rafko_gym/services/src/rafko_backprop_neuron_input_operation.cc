...
void RafkoBackpropNeuronInputOperation::calculate_value(const std::vector<double>& network_input){
  ...
  if(is_network_input){ /* f(x) comes from network input, should not get it from the past */
    ...
    RFASSERT_LOG(
      "operation[{}]: Neuron[{}] Input[{}] f_x = {}(op[{}])", get_operation_index(), neuron_index, neuron_input_index,
  ...
  if(neuron_input_index < (inputs_iterator.cached_size() - 1u)){
    ...
    RFASSERT_LOG(
      "operation[{}]: Neuron[{}] Input[{}] u_x = {}(op[{}])", get_operation_index(), neuron_index, neuron_input_index,
  ...
  RFASSERT_LOG(
    "operation[{}]: Neuron[{}] Input[{}] = {} (collected with {})", get_operation_index(), neuron_index, neuron_input_index,
  ...
  if(is_network_input){
    ...
    kernel_code = rafko_utilities::replace_all_in_string(
      kernel_code, std::regex("==f_x_dependency=="),
        "==op_derivative_array==[==dep_op_index== - (==op_array_size== * ==past_index==)]"
    );
    kernel_code = rafko_utilities::replace_all_in_string(
