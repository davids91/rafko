    ...
    : RafkoBackpropagationOperation(data, network, operation_index,
      ...
      m_isNetworkInput(
          rafko_net::SynapseIterator<rafko_net::InputSynapseInterval>::
              is_index_input(m_inputsIterator[m_neuronInputIndex])),
      m_inputPastIndex(
...
RafkoBackpropNeuronInputOperation::upload_dependencies_to_operations() {
        ...
        {ad_operation_network_input_d,
         {
             rafko_net::SynapseIterator<rafko_net::InputSynapseInterval>::
                 array_index_from_external_index(
                     m_inputsIterator[m_neuronInputIndex]), /* index inside the
                                                               input array for
                                                               the network */
             static_cast<std::uint32_t>(
                 m_weightsIterator[1 + m_neuronInputIndex]), /* weight index to
                                                                be used with the
                                                                input */
  ...
  if (m_neuronInputIndex <
     ...
     * current index, but the elements starting from the next input is as a
     * dependency.
     */
       ...
       [this](std::vector<std::shared_ptr<RafkoBackpropagationOperation>>
         ...
         if (m_isNetworkInput) {
           ...
           RFASSERT_LOG("Neuron input operation[{}]: Registering dependency: "
                        "operation[{}(?)] as network input based on [{}]",
                        get_operation_index(),
         ...
         if (m_neuronInputIndex < (m_inputsIterator.cached_size() - 1u)) {
           ...
           RFASSERT_LOG("Neuron input operation[{}]: Registering dependency: "
                        ...
                        get_operation_index(),
                        dependencies[1]->get_operation_index());
...
void RafkoBackpropNeuronInputOperation::calculate_value(
  ...
  if (m_isNetworkInput) { /* f(x) comes from network input, should not get it
                             from the past */
    RFASSERT(0u == m_inputPastIndex);
    ...
    RFASSERT_LOG("operation[{}]: Neuron[{}] Input[{}] f_x = {}(op[{}])",
                 get_operation_index(), m_neuronIndex, m_neuronInputIndex,
                 weighted_input,
                 m_networkInputDependency->get_operation_index());
...
void RafkoBackpropNeuronInputOperation::calculate_derivative(
  ...
  if (m_isNetworkInput) {
    ...
    f_x_derivative =
        m_networkInputDependency->get_derivative(0u /*past_index*/, d_w_index);
    RFASSERT_LOG("derivative_operation[{}](w[{}]): Neuron[{}] Input[{}]_d f_x "
                 "= {}; f_x_d = {}(d_op[{}])",
                 get_operation_index(), d_w_index, m_neuronIndex,
                 m_neuronInputIndex, f_x_value, f_x_derivative,
                 m_networkInputDependency->get_operation_index());
  } else { /* f(x) comes from Neuron data, may have inputs from the past */
    RFASSERT(static_cast<bool>(m_neuronDataDependency));
    RFASSERT((0u < m_inputPastIndex) ||
...
std::string RafkoBackpropNeuronInputOperation::value_kernel_operation(
  ...
  if (m_isNetworkInput) { /* f(x) comes from network input, should not get it
    ...
    RFASSERT(static_cast<bool>(m_networkInputDependency));
    kernel_source +=
        "f_x_value = " + operations_value_array + "[==f_x_op_index==];\n";
  } else { /* f(x) comes from Neuron data, may have inputs from the past */
    /*!Note: Past values are supposed to be mapped just before the current
     * array, so the negative index should contain the previous run. It the
     * responsibility of the caller to make sure there is no out pf bounds error
     * with these index values.
     */
    RFASSERT(static_cast<bool>(m_neuronDataDependency));
    kernel_source += R"(
      if(==past_index== <= available_memory_slots){
  ...
  /* add the input function */
  kernel_source += ("==op_value_array==[==op_index==] = " +
                    rafko_net::InputFunction::get_kernel_function_for(
                        get_input_function(), "f_x_value", "u_x_value") +
                    ";\n");
  ...
  kernel_source = rafko_utilities::replace_all_in_string(
      ...
      std::to_string(get_weight_descriptor()));
  if (m_isNetworkInput) {
    RFASSERT(0u == m_inputPastIndex);
    RFASSERT(static_cast<bool>(m_networkInputDependency));
    kernel_source = rafko_utilities::replace_all_in_string(
        kernel_source, std::regex("==f_x_op_index=="),
        std::to_string(m_networkInputDependency->get_operation_index()));
  } else {
    RFASSERT(static_cast<bool>(m_neuronDataDependency));
    kernel_source = rafko_utilities::replace_all_in_string(
        kernel_source, std::regex("==f_x_op_index=="),
        std::to_string(m_neuronDataDependency->get_operation_index()));
  }
  if (m_neuronInputIndex < (m_inputsIterator.cached_size() - 1u)) {
    RFASSERT(static_cast<bool>(m_neuronInputDependency));
    kernel_source = rafko_utilities::replace_all_in_string(
        kernel_source, std::regex("==u_x_op_index=="),
        std::to_string(m_neuronInputDependency->get_operation_index()));
  } else { /* the last input starts to collect bias */
    RFASSERT(static_cast<bool>(m_neuronBiasDependency));
    kernel_source = rafko_utilities::replace_all_in_string(
        kernel_source, std::regex("==u_x_op_index=="),
        std::to_string(m_neuronBiasDependency->get_operation_index()));
  }
...
RafkoBackpropNeuronInputOperation::generic_derivative_kernel_operation(
  ...
  kernel_source = rafko_utilities::replace_all_in_string(
      ...
      "==op_derivative_array==[(long int)(==f_x_op_index==) - (long "
      "int)(==op_array_size== * ==past_index==)]");
  kernel_source = rafko_utilities::replace_all_in_string(
