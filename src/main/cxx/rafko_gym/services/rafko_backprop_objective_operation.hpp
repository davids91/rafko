...
public:
  RafkoBackpropObjectiveOperation(RafkoBackpropagationData &data,
      ...
      : RafkoBackpropagationOperation(data, network, operation_index,
        ...
        m_objective(objective), m_outputIndex(output_index),
        m_sampleNumber(sample_number) {}

  std::uint32_t get_sample_number() const { return m_sampleNumber; }
  ...
  void calculate_value(const std::vector<double> & /*network_input*/) override {
    ...
    /*!Note: Value is not being calculated, because they are not of use (as of
     * now.. ) */
    set_value_processed();
  }
  ...
  void calculate_derivative(std::uint32_t d_w_index,
    ...
    set_derivative(
        d_w_index,
        m_objective.get_derivative(
            label_data[m_outputIndex],
            m_featureDependency->get_value(0u /*past_index*/),
            m_featureDependency->get_derivative(0u /*past_index*/, d_w_index),
            static_cast<double>(m_sampleNumber)));
    RFASSERT_LOG(
        "derivative operation[{}](w[{}]): Objective[{}]_d = {} = "
        "derivative({}(label[{}]),{}(op[{}]),{}(d_op),{}(samples))",
        get_operation_index(), d_w_index, m_outputIndex,
        get_derivative(0u /*past_index*/, d_w_index), label_data[m_outputIndex],
        m_outputIndex, m_featureDependency->get_value(0u /*past_index*/),
        m_featureDependency->get_operation_index(),
        m_featureDependency->get_derivative(0u /*past_index*/, d_w_index),
        static_cast<double>(m_sampleNumber));
   ...
   * operation values for forward propagation
   * @param   sample_number                 The number of samples the dataset
   * contains in the mini-batch
   * @param   objective                     The source of the Kernel code for
   * thi Operation
   *
  ...
  static std::string generic_derivative_kernel_operation(
      std::string label_array, std::string operations_value_array,
  ...
  Cost_functions get_cost_type() const { return m_objective.get_cost_type(); }

  std::uint32_t get_label_index() { return m_outputIndex; }
