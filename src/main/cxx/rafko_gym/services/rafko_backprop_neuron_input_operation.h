...
public:
  ...
  void upload_dependencies_to_operations(){
    ...
    if(is_network_input){
      ...
      network_input_dependency = parent.push_dependency(
        ad_operation_network_input_d, input_index_from_neuron_input_index,
        weights_iterator[1 + neuron_input_index]
      );
    }else{ /* if it's not an input, then it's an internal neuron value */
      neuron_data_dependency = parent.push_dependency(
        ad_operation_neuron_spike_d, input_index_from_neuron_input_index
      );
    }
    if(neuron_input_index < (inputs_iterator.cached_size() - 1u)){
      neuron_input_dependency = parent.push_dependency(
  ...
  void calculate(
    ...
    if(neuron_input_index < (inputs_iterator.cached_size() - 1u)){ /* not the last input */
      ...
      if(is_network_input){
        ...
        current_input_derivative = network_input_dependency->get_derivative(run_index, operation_index, d_w_index);
      }else{
        ...
        if(input_index_from_neuron_input_index <= run_index){
          ...
          if(weight_index == d_w_index)current_input_derivative += current_value;
        }else{
          current_value = 0.0;
          current_input_derivative = 0.0;
        }
      }
      set_value(run_index, operation_index, InputFunction::collect(
        network.neuron_array(neuron_index).input_function(),
        weighted_input, neuron_input_dependency->get_value(run_index)
      ));
      set_derivative(run_index, operation_index, d_w_index, InputFunction::get_derivative(
        network.neuron_array(neuron_index).input_function(),
        weighted_input, current_input_derivative,
        neuron_input_dependency->get_value(run_index),
        neuron_input_dependency->get_derivative(run_index, operation_index, d_w_index)
      ));
      ...
      if(is_network_input){ /*!Note: Network input dependency contains weight */
        RFASSERT(network_input_dependency);
        RFASSERT(network_input_dependency->is_processed());
        current_value = network_input[run_index][input_index_from_neuron_input_index];
        current_input_derivative = network_input_dependency->get_derivative(run_index, operation_index, d_w_index);
      }else{
        RFASSERT(neuron_data_dependency);
        RFASSERT(neuron_data_dependency->is_processed());
      ...
      set_value(run_index, current_value);
