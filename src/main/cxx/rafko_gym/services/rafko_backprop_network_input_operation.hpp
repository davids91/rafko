...
public:
  RafkoBackpropNetworkInputOperation(RafkoBackpropagationData &data,
      ...
      : RafkoBackpropagationOperation(data, network, operation_index,
        ...
        m_inputIndex(input_index), m_weightIndex(weight_index) {}
  ~RafkoBackpropNetworkInputOperation() = default;

  std::uint32_t get_weight_index() const { return m_weightIndex; }

  std::uint32_t get_input_index() const { return m_inputIndex; }
  ...
  void calculate_value(const std::vector<double> &network_input) override {
    ...
    set_value(network_input[m_inputIndex] *
              m_network.weight_table(m_weightIndex));
    RFASSERT_LOG(
        "operation[{}]: Network Input[{}]({}) * weight[{}]({}) = {}",
        get_operation_index(), m_inputIndex, network_input[m_inputIndex],
  ...
  void calculate_derivative(std::uint32_t d_w_index,
    ...
    set_derivative(
        d_w_index,
        ((d_w_index == m_weightIndex) ? (network_input[m_inputIndex]) : (0.0)));
    RFASSERT_LOG("derivative operation[{}](w[{}]): Network Input[{}]_d = {}",
                 get_operation_index(), d_w_index, m_inputIndex,
   ...
   */
  static std::string
  generic_value_kernel_operation(std::string network_input_array,
                                 std::string weight_array,
                                 std::string operations_value_array) {
    return (operations_value_array + "[==op_index==] = " + network_input_array +
            "[==network_input_index==]" + " * " + weight_array +
   ...
   */
  static std::string
  generic_derivative_kernel_operation(std::string network_input_array,
