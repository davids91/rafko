...
public:
  ...
  std::string local_declaration_operation() const{
    /*!Note: Weight regularization features have their own locals from @RafkoNetworkFeature::generate_kernel_code
     * but that is enabled by the { } braces inside each switch case in the kernel code generated by @AutoDiffGPUStrategy;
     * hopefully leaving locals in for these operations will not make a significant impact on the footprint of the kernel...
     */
    return "";
  }
  ...
  std::string derivative_kernel_operation(
    ...
    return rafko_net::RafkoNetworkFeature::generate_kernel_code(
      ...
      weight_array, "0"/*weight_start_index*/, operations_derivative_array/* output_array */,
      std::to_string(get_operation_index())/*outputstart_index*/, true/*declare_locals*/
    );
  }
