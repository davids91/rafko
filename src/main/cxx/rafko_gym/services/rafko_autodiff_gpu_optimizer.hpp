 ...
 * update its weights based on it using OpenCL drivers
 */
...
public:
  ...
  void build(const std::shared_ptr<RafkoDataSet> data_set,
             std::shared_ptr<RafkoObjective> objective) override;
   ...
   * on them. The sizes of the buffers are supposed based on the provided
   *          @data_set reference, but actual data uploaded to the GPU from the
   * @data_set only if @refresh_data_set is true
   *
   * @param[in]   data_set            The data set the network is evaluated on
   * @param[in]   force_gpu_upload    if true, the  input values and labels will
   * be reuploaded to GPU based on the dependencies
   */
  void iterate(const RafkoDataSet &data_set,
               bool force_gpu_upload = false) override;

  /**
   * @brief   provides the average gradient for the weight under the given index
   *
   * @return    the average weight gradient for the weight under the given
   * weight index
   */
  ...
  /**
   * @brief     Uploads the input data from the data set into its internal
   * buffers
   *
   ...
   */
  [[nodiscard]] std::vector<cl::Event>
  update_inputs(const RafkoDataSet &data_set);

  /**
   * @brief     Uploads the label data from the data set into its internal
   * buffers
   *
   * @param[in]   data_set                The data set the network is evaluated
   * on
   ...
   */
  [[nodiscard]] std::vector<cl::Event>
  update_labels(const RafkoDataSet &data_set);
