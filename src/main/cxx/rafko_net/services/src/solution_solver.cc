...
,  settings(settings)
{
  ...
  for(std::int32_t row_iterator = 0; row_iterator < solution.cols_size(); ++row_iterator){
    ...
    for(std::uint32_t column_index = 0; column_index < solution.cols(row_iterator); ++column_index){
    ...
    for(std::int32_t row_iterator = 0; row_iterator < solution.cols_size(); ++row_iterator){
      ...
      if( /* Don't use the threadgroup if there is no need for multiple threads.. */
        ...
        while(col_iterator < solution.cols(row_iterator)){
          for(std::uint16_t inner_thread_index = 0; inner_thread_index < settings.get_max_solve_threads(); ++inner_thread_index){
