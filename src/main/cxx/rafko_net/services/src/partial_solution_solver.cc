...
void PartialSolutionSolver::solve_internal(const std::vector<double>& input_data, rafko_utilities::DataRingbuffer<>& output_neuron_data,  std::vector<double>& temp_data) const{
  ...
  for(std::uint16_t neuron_iterator = 0; neuron_iterator < m_partialSolution.output_data().interval_size(); ++neuron_iterator){
    ...
    m_internal_weight_iterator.iterate([&](std::int32_t weight_index){
      if(true == first_weight_in_neuron){ /* as per structure, the first weight is for the spike function */
        ...
        spike_function_weight = m_partialSolution.weight_table(weight_index);
      }else{ /* the next weights are for inputs and biases */
        double new_neuron_input;
        if(m_partialSolution.index_synapse_number(neuron_iterator) > input_synapse_index){ /* Collect input only as long as there is any in the current inner neuron */
          input_index = m_partialSolution.inside_indices(input_synapse_iterator_start + input_synapse_index).starts();
          if(SynapseIterator<>::is_index_input(input_index)){ /* Neuron gets its input from the partial solution input */
            ...
            input_index = m_partialSolution.output_data().starts() + input_index + input_index_offset;
            new_neuron_input = output_neuron_data.get_element(0,input_index);
          }
          ++input_index_offset; /* Step the input index to the next input */
          if(input_index_offset >= m_partialSolution.inside_indices(input_synapse_iterator_start + input_synapse_index).interval_size()){
    ...
    },weight_synapse_iterator_start, m_partialSolution.weight_synapse_number(neuron_iterator));
    weight_synapse_iterator_start += m_partialSolution.weight_synapse_number(neuron_iterator);
    ...
    output_neuron_data.set_element( /* Store the resulting Neuron value */
      0, m_partialSolution.output_data().starts() + neuron_iterator, new_neuron_data
    );
...
bool PartialSolutionSolver::is_valid() const{
  if(
    (0u < m_partialSolution.output_data().interval_size())
    &&(static_cast<int>(m_partialSolution.output_data().interval_size()) == m_partialSolution.index_synapse_number_size())
    &&(static_cast<int>(m_partialSolution.output_data().interval_size()) == m_partialSolution.weight_synapse_number_size())
    &&(static_cast<int>(m_partialSolution.output_data().interval_size()) == m_partialSolution.neuron_transfer_functions_size())
  ){
    std::uint32_t weight_synapse_number = 0;
    std::uint32_t index_synapse_number = 0;

    for(std::uint16_t neuron_iterator = 0u; neuron_iterator < m_partialSolution.output_data().interval_size(); ++neuron_iterator){
      weight_synapse_number += m_partialSolution.weight_synapse_number(neuron_iterator); /* Calculate how many inputs the neuron shall have altogether */
      index_synapse_number += m_partialSolution.index_synapse_number(neuron_iterator); /* Calculate how many inputs the neuron shall have altogether */
    }
    ...
    if((0 < index_synapse_number)&&(0 < weight_synapse_number)){
      ...
      for(std::uint32_t neuron_iterator = 0; neuron_iterator < m_partialSolution.output_data().interval_size(); neuron_iterator++){
        count_of_input_indexes = 0;
        count_of_input_weights = 0;
        for(std::uint32_t input_iterator = 0; input_iterator < m_partialSolution.index_synapse_number(neuron_iterator); ++input_iterator){
          count_of_input_indexes += m_partialSolution.inside_indices(index_synapse_iterator_start + input_iterator).interval_size();
          if( /* If a synapse input in a Neuron points after the neurons index */
            (m_partialSolution.inside_indices(index_synapse_iterator_start + input_iterator).starts()
             + m_partialSolution.inside_indices(index_synapse_iterator_start + input_iterator).interval_size() ) >= neuron_iterator
          ){ /* Self-recurrence is simulated by adding the current data of a neuron as an input into the solution m_partialSolution */
            return false;
          }

          /* Check if the number of weights match the number of input indexes for every Neuron */
          for(std::uint32_t input_iterator = 0; input_iterator < m_partialSolution.weight_synapse_number(neuron_iterator); ++input_iterator){
            count_of_input_weights +=  m_partialSolution.weight_indices(weight_synapse_iterator_start + input_iterator).interval_size();
          }
          weight_synapse_iterator_start += m_partialSolution.weight_synapse_number(neuron_iterator);
          index_synapse_iterator_start += m_partialSolution.index_synapse_number(neuron_iterator);
    ...
    return(
      (static_cast<std::int32_t>(index_synapse_number) == m_partialSolution.inside_indices_size())
      &&(static_cast<std::int32_t>(weight_synapse_number) == m_partialSolution.weight_indices_size())
    );
  }else return false;
}
