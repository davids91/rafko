...
void SolutionBuilder::update(Solution *previous, const RafkoNet &network,
                             bool optimize_to_gpu) {
...
Solution *SolutionBuilder::build(const RafkoNet &net,
                                 google::protobuf::Arena *arena_ptr,
                                 bool optimize_to_gpu) {
  RFASSERT_SCOPE(SOLUTION_BUILD);
  NeuronRouter neuron_router(net);
  Solution *solution =
      google::protobuf::Arena::CreateMessage<Solution>(arena_ptr);
  ...
  if (0 == net.output_neuron_number())
    throw std::runtime_error("Can't build a solution with 0 output Neurons!");
  while (!neuron_router.finished()) { /* Until the whole network is processed */
    if ((!optimize_to_gpu) && (0 == solution->cols_size()))
      ...
      neuron_router.collect_subset(m_settings.get_max_solve_threads(),
                                   m_settings.get_device_max_megabytes(), true);

    remaining_megabytes_in_row = m_settings.get_device_max_megabytes();
    const double max_megabytes_in_one_partial =
        (remaining_megabytes_in_row /
         static_cast<double>(m_settings.get_max_solve_threads()));
    overall_partial_solution_count = solution->partial_solutions_size();

    if (0u < neuron_router.get_subset_size()) {
      for (std::uint32_t partial_index_in_row = 0;
           partial_index_in_row < m_settings.get_max_solve_threads();
           ++partial_index_in_row) {
        ...
        has_neuron = neuron_router.get_first_neuron_index_from_subset(
            current_neuron_index);
        if (has_neuron)
          current_neuron_megabyte_size =
              NeuronInfo::get_neuron_estimated_size_megabytes(
                  net.neuron_array(current_neuron_index));
        else
          break;
        while (/* there are Neurons left to put into this partial */
               ((has_neuron) ||
                (neuron_router.get_first_neuron_index_from_subset(
                    current_neuron_index))) &&
               (/* And the current Neuron is continuing the current partial */
                (0 == this_partial.output_data().interval_size()) ||
                (current_neuron_index ==
                 (get_last_neuron_index_of_partial(this_partial) + 1u)))) {
          if (!has_neuron) /* A new Neuron index was acquired from the router,
                              refresh size info */
            current_neuron_megabyte_size =
                NeuronInfo::get_neuron_estimated_size_megabytes(
                    net.neuron_array(current_neuron_index));
          ...
          if (0u ==
              this_partial.output_data()
                  .interval_size()) /* The first Neuron inside the partial
                                       solution shall determine its start */
            this_partial.mutable_output_data()->set_starts(
                current_neuron_index);

          std::pair<std::uint32_t, std::uint32_t> neuron_input_params =
              partial_builder.add_neuron_to_partial_solution(
                  net, current_neuron_index);
          ...
          std::vector<std::uint32_t> features_solved_by_neuron =
              neuron_router.confirm_first_subset_element_processed(
                  current_neuron_index);
          for (std::uint32_t feature_index : features_solved_by_neuron) {
            *this_partial.add_solved_features() =
                net.neuron_group_features(feature_index);
          }
        ...
        if (/* in case there are no more available Neurons in the subset */
            (0u ==
             neuron_router.get_subset_size()) /* Or the first partial of the
                                                 first row is finished.. */
            ||
            ((!optimize_to_gpu) &&
             (0u ==
              solution->cols_size())) /* ..while optimizing solution to CPU */
        )
          break;
        /*!Note: The first partial of the first row collected Neurons in a
         * non-strict way, so other Neurons might not fit into other partials,
         * because they might have dependencies in this row
         */
      }

      if (solution->partial_solutions_size() >
          static_cast<std::int32_t>(overall_partial_solution_count))
        solution->add_cols(solution->partial_solutions_size() -
                           overall_partial_solution_count);
      neuron_router
          .reset_remaining_subset(); /* Whichever Neuron coudn't fit into the
                                        partial shall have its state reset */
    } /* if(0u < neuron_router.get_subset_size()) */
  }   /* while(!neuron_router.finished()) */

  for (PartialSolution &partial : *solution->mutable_partial_solutions()) {
    std::sort(partial.mutable_solved_features()->begin(),
              partial.mutable_solved_features()->end(),
              [](const FeatureGroup &a, const FeatureGroup &b) {
                return a.feature() < b.feature();
              }); /*!Note: Sorting out FeatureGroups to enforce dependencies,
                     where the larger enum values must be executed later */
  }
  ...
  solution->set_network_memory_length(
      reach_back_max +
      1u); /* Current loop is "0" reachback, so length should be at least 1 */
...
std::string SolutionBuilder::get_kernel_for_solution(
    const Solution &solution, std::string name, std::uint32_t sequence_size,
    std::uint32_t prefill_input_num,
    const rafko_mainframe::RafkoSettings &settings) {
  RFASSERT_LOG("Building GPU kernel for solution");
  /*!Note: Network solve starts from the first memory/sequence slot of the
   * outputs buffer which is calculated from get_global_id(0) and the size of
   * the max(sequence, neuron_memory); UNLESS mode variable input is non-zero.
   * If the mode varaible is non-zero the start of the execution shall start at
   * the end of the last output slice ( the last neuron value array the kernel
   * is supposed to be updating ) and every other output slice before that is
   * considered outputs from the past. The value of the MODE value also tells
   * how many times to run the network.
   */
  std::string source_base = rafko_utilities::random_function +
                            rafko_utilities::atomic_double_add_function + R"(
    void __kernel ==name==(
  ...
  source_base =
      std::regex_replace(source_base, std::regex("==output_start=="),
                         std::to_string((solution.network_memory_length() - 1) *
                                        solution.neuron_number()));
  TransferFunction transfer_function(settings);
  std::uint32_t weight_table_offset =
      1u; /* the first input number is the mode of the solution, so weight table
             starts after that */
  /*!Note: Weight table is supposed to consist of the weight tables of the
   * partial solutions in the order they are inside the @Solution. Inside the
   * kernel this order is used to query the wieghts by position in the device
   * weight table. This note is strictly for one kernel solutions! Networks
   * spanning multiple devices are to re-think this logic.
   */
  std::string neuron_operations;
  std::function<std::string(std::uint32_t, std::string)> past_reach_guard =
      [](std::uint32_t past_reach, std::string content) {
        return "( (current_max_backreach < " + std::to_string(past_reach) +
               " )?(0.0):( " + content + ") )";
      };

  /*!Note: When the network has no memory, backreach counters can not be used to
   * reach the previous value of the Neuron so it would be best to use the
   * currently evaluated label for it (every label except the first reaches the
   * Neurons previous value) EXCEPT it renders single runs incorrect, so it can
   * only be used if the network is under evaluation; since single runs already
   * have the adequate memory allocated, and require no restrictions
   */
  bool feature_locals_declared = false;
  std::vector<std::reference_wrapper<const FeatureGroup>>
      performance_feature_list;
  for (const PartialSolution &partial : solution.partial_solutions()) {
    SynapseIterator<InputSynapseInterval> partial_input_synapses(
        partial.input_data());
    ...
    for (std::uint32_t inner_neuron_index = 0;
         inner_neuron_index < partial.output_data().interval_size();
         ++inner_neuron_index) {
      bool first_weight_synapse_in_neuron = true;
      bool first_input_in_neuron = true;
      std::function<std::string(std::string, std::string, bool)>
          input_function_lambda =
              [&inner_neuron_index, &partial](std::string sum, std::string b,
                                              bool first) -> std::string {
        if (first)
          return sum + " = " + b + ";"; /* The first operation shall set the
                                           value, not update it */
        else
          return (
              sum + "=" +
              InputFunction::get_kernel_function_for(
                  partial.neuron_input_functions(inner_neuron_index), sum, b) +
              ";\n");
      };
      std::uint32_t spike_weight_index;
      std::string inner_neuron_operation = "";
      SynapseIterator<>::skim(
          partial.weight_indices(),
          [&](IndexSynapseInterval weight_synapse) {
            ...
            while ((synapse_weights_done < weight_synapse.interval_size()) &&
                   (input_synapse_index_offset <
                    partial.index_synapse_number(inner_neuron_index))) {
              const std::uint32_t current_input_synapse_size =
                  partial
                      .inside_indices(input_synapse_index +
                                      input_synapse_index_offset)
                      .interval_size();
              std::uint32_t weights_able_to_do = std::min(
                  (weight_synapse.interval_size() - synapse_weights_done),
                  (current_input_synapse_size -
                   input_offset_in_current_synapse));

              /* decide input index start for this synapse */
              std::int32_t input_past_reach =
                  partial
                      .inside_indices(input_synapse_index +
                                      input_synapse_index_offset)
                      .reach_past_loops();
              std::int32_t input_index_start =
                  partial
                      .inside_indices(input_synapse_index +
                                      input_synapse_index_offset)
                      .starts();
              RFASSERT_LOG("InnerNeuron[{} / {}]: ", inner_neuron_index,
                           partial.output_data().interval_size());
              RFASSERT_LOG("synapse_weights_done: {}/{}", synapse_weights_done,
                           weight_synapse.interval_size());
              RFASSERT_LOG("input_synapse_index_offset: {}/{}",
                           input_synapse_index_offset,
                           partial.index_synapse_number(inner_neuron_index));
              RFASSERT_LOG("past: ",
                           partial
              ...
              RFASSERT_LOG("input_offset_in_current_synapse: {}",
                           input_offset_in_current_synapse);
              RFASSERT_LOG("===========================================");
              ...
              if (SynapseIterator<>::is_index_input(input_index_start)) {
                input_index_start =
                    SynapseIterator<>::array_index_from_external_index(
                        input_index_start - input_offset_in_current_synapse);
                input_past_reach = partial_input_synapses
                                       .reach_past_loops<InputSynapseInterval>(
                                           input_index_start);
                weights_able_to_do = std::min(
                    weights_able_to_do,
                    partial_input_synapses.interval_size_of(input_index_start));
              ...
              if (index_points_to_input) {
                ...
                input_string = [input_index_start](std::string addition) {
                  return "inputs[input_start + " + addition + " + " +
                         std::to_string(input_index_start) + "]";
                };
              } else if (0 != input_past_reach) {
                input_string = [input_index_start, input_past_reach,
                                past_reach_guard,
                                &solution](std::string addition) {
                  return past_reach_guard(
                      input_past_reach,
                      "outputs[output_start + " + addition + " + " +
                          std::to_string(input_index_start -
                ...
                input_string = [input_index_start](std::string addition) {
                  return "outputs[output_start + " + addition + " + " +
                         std::to_string(input_index_start) +
                         "]"; /* input doesn't reach to the past */
                };

              /* decide weight string for this synapse */
              std::function<std::string(std::string)> weight_string =
                  [weight_table_offset, weight_synapse,
                   synapse_weights_done](std::string addition) {
                    return "inputs[" + addition + " + " +
                           std::to_string(weight_table_offset +
              ...
              if (first_input_in_neuron) {
                ...
                input_lambda_correction = "neuron_partial_result = (" +
                                          input_string("0") + " *" +
                                          weight_string("0") + " );";
              ...
              std::string neuron_input_operation =
                  ("neuron_partial_result = " +
                   InputFunction::get_kernel_function_for(
                       ...
                       "neuron_partial_result",
                       "(" + input_string("inside_weight_index") + " * " +
                           weight_string("inside_weight_index") + ")") +
                   ";");
              std::string synapse_input_operation =
                  (input_lambda_correction + R"(
            for(int inside_weight_index = ==inside_weight_start==; inside_weight_index < ==inputs_in_synapse==; ++inside_weight_index){
              ...
              synapse_input_operation = std::regex_replace(
                  synapse_input_operation,
                  std::regex("==inside_weight_start=="), inside_weight_start);
              synapse_input_operation = std::regex_replace(
                  synapse_input_operation, std::regex("==inputs_in_synapse=="),
                  std::to_string(weights_able_to_do));
              synapse_input_operation =
                  std::regex_replace(synapse_input_operation,
                                     std::regex("==neuron_input_operation=="),
                                     neuron_input_operation);
              ...
              if (input_offset_in_current_synapse >=
                  current_input_synapse_size) {
                  ...
                  ("neuron_partial_result = " +
                   InputFunction::get_kernel_function_for(
                       ...
                       "inputs[" +
                           ...
                           " + inside_weight_index]") +
                   ";");
              std::string synapse_bias_operation =
                  (input_lambda_correction + R"(
            for(int inside_weight_index = ==inside_weight_start==; inside_weight_index < ==biases_in_synapse==; ++inside_weight_index){
              ...
              synapse_bias_operation = std::regex_replace(
                  synapse_bias_operation, std::regex("==inside_weight_start=="),
                  inside_weight_start);
              synapse_bias_operation = std::regex_replace(
                  synapse_bias_operation, std::regex("==biases_in_synapse=="),
                  std::to_string(weight_synapse.interval_size() -
                                 synapse_weights_done));
              synapse_bias_operation =
                  std::regex_replace(synapse_bias_operation,
                                     std::regex("==neuron_bias_operation=="),
                                     neuron_bias_operation);
              inner_neuron_operation += synapse_bias_operation;
            } /*if(there are no more inputs to pair to the weights --> all
                 inputs are biases)*/
            first_weight_synapse_in_neuron = false;
          },
          weight_synapse_start,
          partial.weight_synapse_number(inner_neuron_index));

      const std::string neuron_index_str =
      ...
      inner_neuron_operation +=
          ("outputs[output_start + " + neuron_index_str + "] = (\n" +
           SpikeFunction::get_kernel_function_for(
               ...
               past_reach_guard(
                   1 /*past_reach*/,
                   std::string("(outputs[output_start + " + neuron_index_str +
                               " - neuron_array_size])")) /* previous_data */,
    ...
    if (0 <
        partial
            .solved_features_size()) { /* if the partial solves any feature */
      for (const FeatureGroup &feature : partial.solved_features()) {
        if (NeuronInfo::is_feature_relevant_to_solution(feature.feature())) {
          RafkoNetworkFeature::add_default_kernel_code_to(
              ...
              "" /*input_array*/, "" /*input_start_index*/, "outputs",
              "output_start", !feature_locals_declared);
          /*!Note: in solution relevant features, only the output array is used,
           * so no need to add any input info */
          feature_locals_declared = true;
        } else if (NeuronInfo::is_feature_relevant_to_performance(
                       feature.feature())) {
    ...
    /*!Note: Because each partial has a different weight table, an offset is
     * required to keep track in case there are multiple partial solutions
     * inside the solution.
     */
  ...
  for (const FeatureGroup &feature : performance_feature_list) {
    bool declare_locals = (/* if the locals have not been declared yet */
                           already_declared_locals.end() ==
                           already_declared_locals.find(feature.feature()));
    RafkoNetworkFeature::add_default_kernel_code_to(
        performance_operations, feature, settings, solution,
        "inputs" /*input_array*/, "1u" /*input_start_index:weight table start*/,
        "outputs" /*output_array*/,
        "output_sizes[0]" /*output_start_index: last output number stores the
                             performance errors */
        ,
        declare_locals); /*!Note: Disentanglement would require the the input to
                            be of the Neuron array */
  ...
  source_base =
      std::regex_replace(source_base, std::regex("==neuron_array_size=="),
                         std::to_string(solution.neuron_number()));
  source_base =
      std::regex_replace(source_base, std::regex("==weight_table_offset=="),
                         std::to_string(weight_table_offset));
  source_base =
      std::regex_replace(source_base, std::regex("==network_memory_slots=="),
                         std::to_string(solution.network_memory_length()));
  source_base = std::regex_replace(source_base, std::regex("==sequence_size=="),
                                   std::to_string(sequence_size));
  source_base = std::regex_replace(
      source_base, std::regex("==neuron_operations=="), neuron_operations);
  source_base =
      std::regex_replace(source_base, std::regex("==prefill_input_num=="),
                         std::to_string(prefill_input_num));
  source_base =
      std::regex_replace(source_base, std::regex("==network_input_size=="),
                         std::to_string(solution.network_input_size()));
  source_base =
      std::regex_replace(source_base, std::regex("==performance_operations=="),
                         performance_operations);
