...
/**
 * @brief      This class Processes a @Solution given in its constructor and
 * handles the distribution of the needed resources for it.
 */
class RAFKO_EXPORT SolutionSolver : public virtual rafko_gym::RafkoAgent {
public:
  SolutionSolver(const Solution *to_solve,
                 const rafko_mainframe::RafkoSettings &settings);

  SolutionSolver(const SolutionSolver &other) = delete; /* Copy constructor */
  SolutionSolver &
  operator=(const SolutionSolver &other) = delete; /* Copy assignment */
  SolutionSolver &
  operator=(SolutionSolver &&other) = delete; /* Move assignment */
   ...
   */
  const rafko_utilities::DataRingbuffer<> &
  get_memory(std::uint32_t thread_index = 0) const {
  ...
  solve(const std::vector<double> &input, bool reset_neuron_data = false,
        std::uint32_t thread_index = 0u) override;

  void set_eval_mode(bool evaluation) override { m_evaluating = evaluation; }
#if (RAFKO_USES_OPENCL)
  /**
   * @brief     Sets the parameters the generated kernel code will be based on.
   * These parameters usually come from a data set
   *
   * @param[in]     sequence_number                 How many sequences are
   * there?
   * @param[in]     sequence_size                   How many feature-label pairs
   * are in a sequence?
   * @param[in]     prefill_inputs_per_sequence     How many prefill inputs are
   * there before a sequence?
   */
  constexpr void
  set_sequence_params(std::uint32_t sequence_number,
  ...
  cl::Program::Sources get_step_sources() const override {
    return {rafko_net::SolutionBuilder::get_kernel_for_solution(
        *m_solution, "agent_solution", m_sequenceSize,
        m_prefillInputsPerSequence, m_settings)};
  }
   ...
   *             3 buffers: mode, weights, and (inputs + prefill) for each
   * evaluated sequence
   *
  ...
  get_input_shapes() const override {
    return {rafko_mainframe::RafkoNBufShape{
        ...
        (m_sequencesEvaluating * (m_sequenceSize + m_prefillInputsPerSequence) *
         m_solution->network_input_size())}};
  }

  /**
   * @brief      Provides the output dimension of the agent, which consist of
   *             1 buffer: Neuron outputs for each evaluated sequence or network
   * memory
   *
   * @return     Vector of dimensions in order of @get_step_sources and
   * @get_step_names Agent output structure: {{ used bytes for execution, used
   * bytes for performance feature error summary }}
   */
  std::vector<rafko_mainframe::RafkoNBufShape>
  get_output_shapes() const override {
    const std::size_t bytes_used =
        (std::max(m_sequencesEvaluating,
                  1u) /* number of sequences to evaluate */
         *
         std::max(
             2u,
             std::max(/* number of labels per sequence */
                      m_solution->network_memory_length(),
                      (m_sequenceSize +
                       m_prefillInputsPerSequence))) /*!Note: at least 2 slots
                                                        are needed because the
                                                        Spike Function takes its
                                                        input from the previous
                                                        slot always */
         * m_solution->neuron_number() /* number of values per label */
        );
    return {rafko_mainframe::RafkoNBufShape{bytes_used,
                                            1u /* for performance error */}};
  }
...
private:
  ...
  std::vector<rafko_utilities::DataRingbuffer<>>
      m_neuronValueBuffers; /* One rafko_utilities::DataRingbuffer per thread */
  ...
  /**
   * @brief     Updates the stored @Solution pointer and rebuilds the underlying
...
public:
  ...
  public:
    Factory(const RafkoNet &network,
    ...
    /**
     * @brief     Provides const access to the used weight adapter so
     * information might be queried based on it
     *
    ...
    /**
     * @brief     Updates the stored solution with the weights from the stored
    ...
    /**
     * @brief     Builds a SolutionSolver and produces a pointer to it, based on
     * its stored members
     *
     * param[in]    rebuild_solution    Creates a new @Solution object and
     * stores it as reference param[in]    swap_solution       When true, no new
     * Solution is stored, instead the last built solution is swapped with the
     * newly built one.
     *
     * @return    Ownership and pointer of the built solver
     */
    std::shared_ptr<SolutionSolver> build(bool rebuild_solution = false,
                                          bool swap_solution = false);
