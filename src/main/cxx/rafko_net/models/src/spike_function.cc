...
double SpikeFunction::get_value(Spike_functions function, double parameter,
  ...
  case spike_function_p:
    return previous_data + ((new_data - previous_data) * parameter);
  case spike_function_amplify_value:
    return new_data * parameter;
  default:
    throw std::runtime_error(
        "Unknown spike function requested for calculation!");
  }
}
...
double SpikeFunction::get_derivative_for_w(/* means: x = w; new_data = g(x);
  ...
  case spike_function_memory: /* S(x,w,f(x),g(x)) = w * f(x) + g(x) - w * g(x)
    ...
    return (parameter * previous_data_d) + previous_data -
           (parameter * new_data_d) + new_data_d - new_data;
  case spike_function_p: /* S(x,w,f(x),g(x)) = f(x) + g(x) * w - f(x) * w */
    /* S'(x,w,f(x),g(x)) = -w * f'(x) + f'(x) - f(w) + w * g'(x) + g(x) */
    return -parameter * previous_data_d + previous_data_d - previous_data +
  ...
  default:
    throw std::runtime_error(
        "Unknown spike function requested for derivative calculation!");
  }
}

double SpikeFunction::get_derivative_not_for_w(/* means: x = w; new_data = g(x);
                                                  previous_data = f(x) */
                                               Spike_functions function,
                                               double parameter,
                                               double previous_data_d,
  ...
  case spike_function_memory: /* S(x,w,f(x),g(x)) = w * f(x) + g(x) - w * g(x)
    ...
    return (parameter * previous_data_d) - (parameter * new_data_d) +
           new_data_d;
  case spike_function_p: /* S(x,w,f(x),g(x)) = f(x) + g(x) * w - f(x) * w */
...
Spike_functions SpikeFunction::next(std::set<Spike_functions> range) {
  ...
  Spike_functions candidate =
      static_cast<Spike_functions>(rand() % Spike_functions_ARRAYSIZE);
  while (!Spike_functions_IsValid(candidate) ||
...
#if (RAFKO_USES_OPENCL)
std::string SpikeFunction::get_kernel_function_for(Spike_functions function,
  ...
  case spike_function_none:
    return "(" + new_data + ")";
  case spike_function_memory:
    return "(((" + previous_data + ") * " + parameter + ") + ((" + new_data +
           ") * (1.0 - " + parameter + ")))";
  case spike_function_p:
    return "(" + previous_data + "+((" + new_data + "-" + previous_data +
           ") *" + parameter + ")";
  case spike_function_amplify_value:
    return "(" + new_data + "*" + parameter + ")";
  default:
    throw std::runtime_error(
        "Unidentified spike function queried for kernel code!");
  }
}
...
std::string SpikeFunction::get_all_kernel_value_functions(
  ...
  code = rafko_utilities::replace_all_in_string(
      code, std::regex("==parameter=="), parameter);
  code = rafko_utilities::replace_all_in_string(
      code, std::regex("==new_data=="), new_data);
  code = rafko_utilities::replace_all_in_string(
...
std::string SpikeFunction::get_derivative_kernel_for_w(
    Spike_functions function, std::string parameter, std::string previous_data,
    std::string previous_data_d, std::string new_data, std::string new_data_d) {
  ...
  case spike_function_none:
    return new_data_d;
  case spike_function_memory:
    return ("(" + parameter_ + "*" + previous_data_d_ + ") + " + previous_data +
            " - (" + parameter_ + " * " + new_data_d_ + ") + " + new_data_d_ +
            " - " + new_data);
  case spike_function_p:
    return ("-" + parameter_ + " * " + previous_data_d_ + " + " +
            previous_data_d_ + " - " + previous_data + " + " + parameter_ +
            " * " + new_data_d_ + " + " + new_data);
  case spike_function_amplify_value:
...
std::string SpikeFunction::get_derivative_kernel_not_for_w(
  ...
  case spike_function_none:
    return new_data_d_;
  case spike_function_memory:
    return ("(" + parameter_ + "*" + previous_data_d_ + ") - (" + parameter_ +
            "*" + new_data_d_ + ")" + "+" + new_data_d_);
  case spike_function_p:
    return "(" + parameter_ + "*" + new_data_d_ + ") - ((" + parameter_ +
           "-1.0) * " + previous_data_d_ + ")";
  case spike_function_amplify_value:
...
std::string SpikeFunction::get_kernel_enum_for(Spike_functions function) {
  switch (function) {
  case spike_function_none:
    return "spike_function_none";
  case spike_function_memory:
    return "spike_function_memory";
  case spike_function_p:
    return "spike_function_p";
  case spike_function_amplify_value:
    return "spike_function_amplify_value";
  default:
    throw std::runtime_error(
        "Unidentified spike function queried for information!");
  }
}
