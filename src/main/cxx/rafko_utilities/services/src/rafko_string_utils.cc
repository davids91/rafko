...
std::string replace_all_in_string(std::string input_text,
  ...
  std::uint32_t matches_count =
      std::distance(/* https://stackoverflow.com/questions/8283735/count-number-of-matches
                     */
                    std::sregex_iterator(text.begin(), text.end(),
                                         regex_to_replace),
                    std::sregex_iterator());
...
std::string escape_string(std::string characters_to_escape, std::string s) {
  ...
  for (const char &c : characters_to_escape) {
    result = replace_all_in_string(result, std::regex(std::to_string(c)),
                                   "\\" + std::to_string(c));
  }
  return result;
}
