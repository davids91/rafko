...
void ThreadGroup::start_and_block(
    const std::function<void(std::uint32_t)> &function) const {
  ...
  { /* wait until the work is done */
    std::unique_lock<std::mutex> my_lock(m_stateMutex);
    m_synchroniser.wait(
        my_lock, [this]() { return (m_threads.size() <= m_threadsReady); });
  }
  { /* set appropriate state */
    std::unique_lock<std::mutex> my_lock(m_stateMutex);
    m_state.store(Idle);
  }
  m_synchroniser.notify_all(); /* Notify worker threads that the main thread is
                                  finished */

  { /* wait until all threads are notified */
    std::unique_lock<std::mutex> my_lock(m_stateMutex);
    m_synchroniser.wait(my_lock, [this]() {
      return (0u == m_threadsReady); /* All threads are notified once the
                                        @threads_ready variable is zero again */
    });
  }
}
...
void ThreadGroup::worker(std::uint32_t thread_index) {
  while (End != m_state.load()) { /* Until the pool is stopped */
    {                             /* Wait until main thread triggers a task */
      std::unique_lock<std::mutex> my_lock(m_stateMutex);
      m_synchroniser.wait(my_lock,
                          [this]() { return (Idle != m_state.load()); });
    }
    if (End !=
        m_state.load()) { /* In case there are still tasks to execute.. */
      ...
      m_synchroniser
          .notify_all(); /* Notify main thread that this thread  is finsished */

      { /* Wait until main thread is closing the iteration */
        std::unique_lock<std::mutex> my_lock(m_stateMutex);
        m_synchroniser.wait(my_lock,
                            [this]() { return (Start != m_state.load()); });
      }
