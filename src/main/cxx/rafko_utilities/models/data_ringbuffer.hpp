...
/**
 * @brief      This class describes a ringbuffer designed to store the Memory of
 * a Neural Network. At the life-cycle of a Neural network one solution counts
 * as a "loop", where the data of the neurons shall be calculated and copied
 * into an array. The array stores the activation values of the Neurons from
 * this loop, and the previous loops as well. This class provides that array. At
 * every loop, it provides Read/Write Access to the latest element in the buffer
 * and read access to the previous data of the previous. At the start of each
 * loop, it copies the data from the previous loops into the current one. The
 * data under the current loop needs to keep its contents, while the data from
 * previous loops also need to be stored.
 */
template <typename Content = std::vector<double>>
class RAFKO_EXPORT DataRingbuffer {
public:
  DataRingbuffer(std::uint32_t buffer_number,
                 std::function<void(Content &)> initer)
      : m_data(buffer_number) {
    for (Content &buffer : m_data)
      initer(buffer);
  }

  /**
   * @brief      Store the current data and move the iterator forward for the
   * next one
   */
  void copy_step() {
    m_currentIndex = (m_currentIndex + 1) % (m_data.size());
    if (1 < m_data.size())
      std::copy(get_element(1).begin(), get_element(1).end(),
                get_element(0).begin());
  }
  ...
  void reset() {
    m_currentIndex =
        (m_data.size() -
         1); /* Set the current index into the last index, so at the next @ */
    reset(m_data);
  }
  ...
  void copy_latest(const DataRingbuffer &other) {
    std::copy(other.get_element(0).begin(), other.get_element(0).end(),
              get_element(0).begin());
  }
   ...
   * @param[in]  data_index  The index of the data point to retrive in the
   * buffer
   *
   * @return     The value of the data in the   given index parameters
   */
  typename Content::value_type get_element(std::uint32_t past_index,
                                           std::uint32_t data_index) const {
    if ((m_data.size() > past_index) && (m_data[0].size() > data_index))
      return get_element(past_index)[data_index];
    else
      throw std::runtime_error("Ringbuffer data index out of bounds!");
  }
  ...
  /**
   * @brief      Sets the data element under the given indices to the provided
   * value
   *
   ...
   */
  void set_element(std::uint32_t past_index, std::uint32_t data_index,
                   typename Content::value_type value) {
    if ((m_data.size() > past_index) && (m_data[0].size() > data_index))
   ...
   */
  std::uint32_t get_sequence_size() const { return m_data.size(); }

  /**
   * @brief      Calculates the index to reach the neuron data at the
   * @sequence_index th evaluation of a data sample which was the last
   * @past_index th loop.
   *
   * @param[in]  sequence_index    The sequence index
   * @param[in]  reach_past_loops  The number of loops to reach back in the
   * sequence
   *
   * @return     The buffer index.
   */
  std::int32_t get_sequence_index(std::uint32_t sequence_index,
   ...
   */
  std::uint32_t buffer_size() const { return m_data[0].size(); }
   ...
   */
  std::uint32_t buffer_number() const { return m_data.size(); }
...
private:
  ...
  std::uint32_t get_buffer_index(std::uint32_t past_index) const {
    if (m_data.size() <= past_index)
      throw std::runtime_error("Older data queried, than memory capacity.");
    if (past_index > m_currentIndex)
      return (m_data.size() + m_currentIndex - past_index);
    else
      return (m_currentIndex - past_index);
  }

  /**
   * @brief resets different underlying vector types
   */
  constexpr void reset(double buf) { buf = (0.0); }
