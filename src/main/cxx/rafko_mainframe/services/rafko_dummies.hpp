...
public:
  RafkoDummyEnvironment(std::uint32_t input_size = 1u,
  ...
  const std::vector<std::vector<double>> &get_input_samples() const override {
    return m_dummyInputs;
  }
  const std::vector<double> &
  ...
  const std::vector<std::vector<double>> &get_label_samples() const override {
    return m_dummyLabels;
  }
  std::uint32_t get_feature_size() const override {
    return m_dummyLabels[0].size();
  }
...
public:
  RafkoDummyGPUStrategyPhase(RafkoNBufShape input_shape,
                             RafkoNBufShape output_shape)
      : m_inputShape(input_shape), m_outputShape(output_shape) {}
