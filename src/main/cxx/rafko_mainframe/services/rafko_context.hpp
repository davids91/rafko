...
/**
 * @brief      The interface for the main context of the Rafko Deep learning
 * service. It encapsulates a Network as its central point, and provides methods
 * to refine it and solve it.
 */
...
public:
  ...
  /**
   * @brief          Accepts a data set to base network evaluation on top of and
   * takes ownership of it!
   *
   * @param[in]      environment    An environment ready to be moved inside the
   * context
   */
  virtual void
  set_data_set(std::shared_ptr<rafko_gym::RafkoDataSet> environment) = 0;

  /**
   * @brief          Accepts an objective function to base network evaluation on
   * top of and takes ownership of it!
   *
   * @param[in]      objective    An objective function ready to be moved inside
   * the context
   */
  virtual void
  set_objective(std::shared_ptr<rafko_gym::RafkoObjective> objective) = 0;

  /**
   * @brief          Accepts a weight updater type to make handle the weight
   * updates
   *
   * @param[in]      objective    An objective function ready to be moved inside
   * the context
   ...
   *
   * @param[in]      weight_index     The index inside the networks weight table
   * to be modified
   * @param[in]      weight_value     The value to set the new weight to
   */
  virtual void set_network_weight(std::uint32_t weight_index,
  ...
  /**
   * @brief          Applies a weight change based on the provided weights
   * vector
   *
  ...
  /**
   * @brief          Evaluates installed agents and returns with its
   * error/fittness value
  ...
  /**
   * @brief          Evaluates installed agents in a stochastic manner and
   * returns with its error/fittness value
   *
   * @param[in]      to_seed              A helper value to make Stochastic
   * evaluation deterministicly reproducible
   * @param[in]      seed_value           A helper value to make Stochastic
   * evaluation deterministicly reproducible
   * @param[in]      force_gpu_upload     If set true, data in stored objects
   * are uploaded to GPU regardless of previous uploads Applies only to
   * implementations targeting GPUs
   *
   * @return         The resulting error/fitness value summary of the evaluation
   */
  virtual double stochastic_evaluation(bool to_seed = false,
                                       std::uint32_t seed_value = 0u,
                                       bool force_gpu_upload = false) = 0;
  ...
  solve(const std::vector<double> &input, bool reset_neuron_data = true,
        std::uint32_t thread_index = 0) = 0;

  /**
   * @brief     Solves the enclosed network for the whole of the included
   * environment.
   *
   * @param         output      The buffer to store the data in. Sizes must be
   * set to fit the output exactly
   * @param[in]     isolated    Set to true, if the buffers are to be resetted
   * before solving the data set Number of sequences evaluated may be limited by
   * the available threads!
   */
  virtual void solve_data_set(std::vector<std::vector<double>> &output,
                              bool isolated) = 0;
