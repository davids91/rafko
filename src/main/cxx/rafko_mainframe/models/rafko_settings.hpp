...
public:
  ...
  std::uint32_t get_minibatch_size() const { return m_hypers.minibatch_size(); }
  ...
  bool get_training_strategy(rafko_gym::Training_strategy strategy) const {
    return (0u < (static_cast<std::uint32_t>(m_hypers.training_strategies()) &
                  static_cast<std::uint32_t>(strategy)));
  }

  double get_alpha() const { return m_hypers.alpha(); }

  double get_beta() const { return m_hypers.beta(); }

  double get_beta_2() const { return m_hypers.beta(); }

  double get_gamma() const { return m_hypers.gamma(); }

  double get_delta() const { return m_hypers.delta(); }

  double get_delta_2() const { return m_hypers.delta_2(); }

  double get_epsilon() const { return m_hypers.epsilon(); }

  constexpr double get_sqrt_epsilon() const { return m_sqrtEpsilon; }

  double get_zetta() const { return m_hypers.zetta(); }

  double get_lambda() const { return m_hypers.lambda(); }
  ...
  RafkoSettings &set_max_solve_threads(double max_solve_threads) {
    ...
    m_sqrtOfSolveThreads = static_cast<std::uint16_t>(
        std::max((1.0), std::sqrt(static_cast<double>(m_maxSolveThreads))));
    return *this;
  }
  ...
  RafkoSettings &
  set_max_processing_threads(std::uint16_t max_processing_threads) {
  ...
  RafkoSettings &
  set_training_relevant_loop_count(std::uint32_t training_relevant_loop_count) {
    m_hypers.set_training_relevant_loop_count(training_relevant_loop_count);
    return *this;
  }

  constexpr RafkoSettings &
  set_device_max_megabytes(double device_max_megabytes) {
  ...
  RafkoSettings &set_training_strategy(rafko_gym::Training_strategy strategy,
                                       bool enable) {
    if (enable) {
      m_hypers.set_training_strategies(
          static_cast<rafko_gym::Training_strategy>(
              static_cast<std::uint32_t>(m_hypers.training_strategies()) |
              static_cast<std::uint32_t>(strategy)));
    } else {
      m_hypers.set_training_strategies(
          static_cast<rafko_gym::Training_strategy>(
  ...
  RafkoSettings &set_learning_rate_decay(
      std::vector<std::pair<std::uint32_t, double>> &&iteration_with_value) {
  ...
  RafkoSettings() {
    ...
    m_hypers.set_training_strategies(
        rafko_gym::Training_strategy::training_strategy_unknown);
    ...
    m_hypers.set_epsilon(
        1e-8); /* very small positive value almost greater, than (0.0) */
...
private:
  ...
  rafko_gym::TrainingHyperparameters m_hypers =
      rafko_gym::TrainingHyperparameters();
  ...
  void calculate_learning_rate_decay() {
    ...
    for (std::pair<std::uint32_t, double> decay : m_learningRateDecay) {
      ...
      m_learningRateWithDecay.push_back(
          {std::get<std::uint32_t>(decay), learning_rate});
    }
  }
};
