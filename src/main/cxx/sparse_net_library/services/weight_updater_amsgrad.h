...
public:
  Weight_updater_amsgrad(SparseNet& sparse_net, Service_context& service_context)
  ...
  void iterate(const vector<sdouble32>& gradients,Solution& solution){
    ...
    for(uint32 weight_index = 0; weight_index < moment.size(); ++weight_index){
      moment[weight_index] = (
        (context.get_beta() * moment[weight_index])
        + ( ((double_literal(1.0) - context.get_beta()) * gradients[weight_index]) )
      );
      raw_moment = (
        (context.get_beta_2() * raw_moment_max[weight_index])
        + (
          ((double_literal(1.0) - context.get_beta_2())
...
private:
  sdouble32 get_new_velocity(uint32 weight_index, const vector<sdouble32>& gradients){
    return ( context.get_step_size() * moment[weight_index] / ( std::sqrt(raw_moment_max[weight_index]) + context.get_epsilon() ) );
  }
