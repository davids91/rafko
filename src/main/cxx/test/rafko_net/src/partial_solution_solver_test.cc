 ...
 *   - 2 inputs
 *   - 2 Neurons: The first neuron has the inputs and the second has the first
 * neuron
...
TEST_CASE("Solving an artificial partial_solution detail",
          "[solve][partial-solution][manual]") {
  rafko_mainframe::RafkoSettings settings;
  rafko_utilities::DataRingbuffer<> neuron_data(
      1u, [](std::vector<double> &element) { element.resize(2u); });
  ...
  rafko_test::manual_2_neuron_partial_solution(partial_solution,
                                               network_inputs.size());
  ...
  *partial_solution.add_input_data() = temp_synapse_interval;
  ...
  rafko_test::manual_2_neuron_result(network_inputs, expected_neuron_output,
                                     partial_solution);
  CHECK(
      Catch::Approx(neuron_data.get_element(0, 1)).epsilon(0.00000000000001) ==
      expected_neuron_output[1]);

  /* The result should change in accordance with the parameters */
  srand(time(nullptr));
  for (std::uint8_t variant_iterator = 0; variant_iterator < 100u;
       variant_iterator++) {
    rafko_net::SynapseIterator<>::iterate(
        partial_solution.weight_indices(),
        [&](std::int32_t neuron_weight_index) {
          partial_solution.set_weight_table(
              neuron_weight_index, static_cast<double>(rand() % 11) / (10.0));
        },
    ...
    rafko_test::manual_2_neuron_result(network_inputs, expected_neuron_output,
                                       partial_solution);
    CHECK(Catch::Approx(neuron_data.get_element(0, 1))
              .epsilon(0.00000000000001) == expected_neuron_output[1]);
    ...
    partial_solution.set_neuron_transfer_functions(
        rand() % (partial_solution.neuron_transfer_functions_size()),
        rafko_net::TransferFunction::next());
    solver.solve(network_inputs, neuron_data);
    rafko_test::manual_2_neuron_result(network_inputs, expected_neuron_output,
                                       partial_solution);
    REQUIRE(Catch::Approx(neuron_data.get_element(0, 1))
                .epsilon(0.00000000000001) == expected_neuron_output[1]);
  }
}
 ...
 * - define the partial solution so every neuon gives back the corresponding
 * input
 * - see if the input is collected correctly
 */
TEST_CASE("Test Partial solution input collection",
          "[solve][partial-solution][input_collection]") {
  rafko_mainframe::RafkoSettings settings;
  rafko_net::PartialSolution partial_solution;
  std::vector<double> network_inputs = {(1.9), (2.8), (3.7), (4.6), (5.5),
                                        (6.4), (7.3), (8.2), (9.1), (10.0)};
  rafko_net::IndexSynapseInterval temp_index_interval;
  rafko_net::InputSynapseInterval temp_input_interval;
  rafko_utilities::DataRingbuffer<> neuron_data(
      1u, [&network_inputs](std::vector<double> &element) {
  ...
  *partial_solution.mutable_output_data() = temp_index_interval;
  partial_solution.add_weight_table(
      (0.0)); /* A weight for the spike function */
  for (std::uint32_t i = 0; i < network_inputs.size(); ++i) {
    ...
    partial_solution.add_neuron_spike_functions(
        rafko_net::spike_function_memory);

    partial_solution.add_index_synapse_number(
        1); /* 1 synapse for indexes and 1 for weights */
    temp_input_interval.set_starts(
        rafko_net::SynapseIterator<>::external_index_from_array_index(i));
    temp_input_interval.set_interval_size(
        1u); /* Input index synapse starts at the beginning of the data and goes
                on for an interval of 1 */
    *partial_solution.add_inside_indices() = temp_input_interval;
    ...
    temp_index_interval.set_interval_size(
        1u + 1u); /* weight1 + 1 weight for the spike function*/
  ...
  *partial_solution.add_input_data() = temp_input_interval;
  ...
  temp_input_interval.set_starts(
      rafko_net::SynapseIterator<>::external_index_from_array_index(3));
  temp_input_interval.set_interval_size(3);
  *partial_solution.add_input_data() = temp_input_interval;

  /* Elements from 6 to 8 */
  temp_input_interval.set_starts(
      rafko_net::SynapseIterator<>::external_index_from_array_index(6));
  temp_input_interval.set_interval_size(2);
  *partial_solution.add_input_data() = temp_input_interval;

  /* Elements from 8 to 9 ( to the end ) */
  temp_input_interval.set_starts(
      rafko_net::SynapseIterator<>::external_index_from_array_index(8));
  temp_input_interval.set_interval_size(2);
  *partial_solution.add_input_data() = temp_input_interval;
  ...
  solver.solve(network_inputs,
               neuron_data); /* Since the network just spits the inputs back out
                                so the input collection is testable through it*/
  for (std::uint32_t i = 0; i < network_inputs.size(); ++i) {
    REQUIRE(Catch::Approx(network_inputs[i]).epsilon(0.00000000000001) ==
            neuron_data.get_element(0, i));
  }
}
