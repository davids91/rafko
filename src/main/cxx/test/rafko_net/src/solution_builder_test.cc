...
/*###############################################################################################
 * Testing Solution generation using the @RafkoNetBuilder and the
 * @SolutionBuilder
 * */
rafko_net::Solution *
test_solution_builder_manually(google::protobuf::Arena *arena,
                               double device_max_megabytes,
  ...
  rafko_mainframe::RafkoSettings settings =
      rafko_mainframe::RafkoSettings()
          ...
          .set_device_max_megabytes(device_max_megabytes)
          .set_arena_ptr(arena);

  rafko_net::RafkoNetBuilder builder = rafko_net::RafkoNetBuilder(settings);

  builder.input_size(50).expected_input_range(5.0).output_neuron_number(
      net_structure.back());

  std::uint32_t layer_index = rand() % net_structure.size();
  if (recursion)
    builder.add_neuron_recurrence(layer_index,
  ...
  for (std::int32_t neuron_iterator = 0;
       neuron_iterator < net->neuron_array_size(); ++neuron_iterator) {
    found = false;
    for (std::int32_t partial_iterator = 0;
         partial_iterator < solution->partial_solutions_size();
         ++partial_iterator) {
      REQUIRE(0u < solution->partial_solutions(partial_iterator)
                       .output_data()
                       .interval_size());
      for (std::uint32_t internal_neuron_iterator = 0;
           internal_neuron_iterator <
           solution->partial_solutions(partial_iterator)
               .output_data()
               .interval_size();
           ++internal_neuron_iterator) {
        if (static_cast<std::int32_t>(
                solution->partial_solutions(partial_iterator)
                    .output_data()
                    .starts() +
                internal_neuron_iterator) == neuron_iterator) {
          found = true;
  ...
  Solution_search_over:
    REQUIRE(
        true ==
        found); /* Found the Neuron index from the net in the result solution */
  }

  /* Test if the inputs of the partial in the first row only contain input
   * indexes */
...
TEST_CASE("Building a solution from a small net",
          "[build][small][build-only]") {
  ...
  space_used_megabytes = solution->SpaceUsedLong() /* Bytes */ /
                         (1024.0) /* KB */ / (1024.0) /* MB */;
  solution.reset();

  /* test it again, but with intentionally dividing the partial solutions by
   * multiple numbers */
  solution =
      std::unique_ptr<rafko_net::Solution>(test_solution_builder_manually(
...
TEST_CASE("Building a solution from a bigger net", "[build][build-only]") {
  ...
  solution = std::unique_ptr<rafko_net::Solution>(
      test_solution_builder_manually(nullptr, space_used_megabytes / 5.0,
                                     {20, 20, 30, 10, 5}, false, false));
  REQUIRE(nullptr != solution);
  REQUIRE(0 < solution->SpaceUsedLong());
  solution.reset();
