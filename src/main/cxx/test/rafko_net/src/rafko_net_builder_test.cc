 ...
 */
rafko_net::RafkoNet *
test_net_builder_fully_connected(google::protobuf::Arena *arena) {
  ...
  rafko_net::RafkoNet *network(
      rafko_net::RafkoNetBuilder(settings)
          ...
          .create_layers({2, 3, 2}, {{rafko_net::transfer_function_identity},
  ...
  REQUIRE(((5 * 2 + 2 * 2) + (2 * 3 + 3 * 2) + (2 * 3 + 2 * 2)) ==
  ...
  for (int i = 0; i < 7; ++i) {
    REQUIRE(true ==
            rafko_net::NeuronInfo::is_neuron_valid(network->neuron_array(i)));

    /* Check the indexing */
    REQUIRE(0 < network->neuron_array(i).input_indices_size());
    CHECK(1 ==
          network->neuron_array(i)
              .input_indices_size()); /* One synapse ==> the previous layer */
    number_of_input_indexes = 0;
    for (std::int32_t index_syn_iter = 0;
         index_syn_iter < network->neuron_array(i).input_indices_size();
         ++index_syn_iter) {
      REQUIRE(/* Every index synapse element has to point inside the neuron
                 array */
              network->neuron_array_size() >
              static_cast<std::int32_t>(network->neuron_array(i)
                                            .input_indices(index_syn_iter)
                                            .starts() +
                                        network->neuron_array(i)
                                            .input_indices(index_syn_iter)
                                            .interval_size()));
      number_of_input_indexes += network->neuron_array(i)
                                     .input_indices(index_syn_iter)
                                     .interval_size();
    }
    ...
    for (std::int32_t weight_syn_iterator = 0;
         weight_syn_iterator < network->neuron_array(i).input_weights_size();
         ++weight_syn_iterator) {
      /* Weights */
      REQUIRE(/* Every weight synapse element has to point inside the weight
                 table array */
              network
                  ->weight_table_size() >= /* Equality is permitted here,
                                              because the interval iterates from
                                              (start) to (start + size - 1) */
              static_cast<std::int32_t>(network->neuron_array(i)
                                            .input_weights(weight_syn_iterator)
                                            .starts() +
                                        network->neuron_array(i)
                                            .input_weights(weight_syn_iterator)
                                            .interval_size()));
      for (std::uint32_t weight_iterator = 0;
           weight_iterator < network->neuron_array(i)
                                 .input_weights(weight_syn_iterator)
                                 .interval_size();
           ++weight_iterator) {
        CHECK(/* The weights of the Neuron has to be inbetween (-1;1) */
              -(1.0) <=
              network->weight_table(network->neuron_array(i)
                                        .input_weights(weight_syn_iterator)
                                        .starts() +
                                    weight_iterator));
        CHECK((1.0) >=
      ...
      number_of_input_weights += network->neuron_array(i)
                                     .input_weights(weight_syn_iterator)
                                     .interval_size();
    }
   ...
   */
  CHECK(rafko_net::SynapseIterator<>::external_index_from_array_index(0) ==
        network->neuron_array(0)
            .input_indices(0)
            .starts()); /* 0th Input, translated using SynapseIterator<> */
  CHECK(rafko_net::SynapseIterator<>::external_index_from_array_index(0) ==
        network->neuron_array(1).input_indices(0).starts());

  /* The input Layer should have Identity transfer function according to
   * configuration */
  CHECK(rafko_net::transfer_function_identity ==
  ...
  CHECK(0 == network->neuron_array(2)
                 .input_indices(0)
                 .starts()); /* 0th Neuron, because neuron index >=
                                network->input_neuron_number() */
  CHECK(0 == network->neuron_array(3).input_indices(0).starts());
  CHECK(0 == network->neuron_array(4).input_indices(0).starts());

  /* The Hidden Layer should have either transfer_function_relu or
   * transfer_function_selu according to the configuration */
  CHECK(((rafko_net::transfer_function_relu ==
          network->neuron_array(2).transfer_function()) ||
         (rafko_net::transfer_function_selu ==
          network->neuron_array(2).transfer_function())));

  CHECK(((rafko_net::transfer_function_relu ==
          network->neuron_array(3).transfer_function()) ||
         (rafko_net::transfer_function_selu ==
          network->neuron_array(3).transfer_function())));

  CHECK(((rafko_net::transfer_function_relu ==
          network->neuron_array(4).transfer_function()) ||
         (rafko_net::transfer_function_selu ==
          network->neuron_array(4).transfer_function())));
  ...
  /* Output Neurons should their synapse start at the 2nd Neuron (Previous layer
   * start) */
  CHECK(2 == network->neuron_array(5).input_indices(0).starts());
  CHECK(2 == network->neuron_array(6).input_indices(0).starts());

  /* The Output Layer should have either transfer_function_sigmoid or
   * transfer_function_tanh according to the configuration */
  CHECK(((rafko_net::transfer_function_sigmoid ==
          network->neuron_array(5).transfer_function()) ||
         (rafko_net::transfer_function_tanh ==
          network->neuron_array(5).transfer_function())));

  CHECK(((rafko_net::transfer_function_sigmoid ==
          network->neuron_array(6).transfer_function()) ||
         (rafko_net::transfer_function_tanh ==
          network->neuron_array(6).transfer_function())));
  return network;
}
...
TEST_CASE(
    "Builder to construct Fully Connected Net correctly through the interface",
    "[build][small]") {
...
TEST_CASE("Builder to construct Fully Connected Net correctly through the "
          "interface with arena",
          "[build][arena][small]") {
...
TEST_CASE("Testing builder for adding different features to different layers",
          "[build][features]") {
  google::protobuf::Arena arena;
  rafko_mainframe::RafkoSettings settings =
      rafko_mainframe::RafkoSettings().set_arena_ptr(&arena);
  for (std::uint32_t variant = 0u; variant < 10u; ++variant) {
    ...
    std::unordered_map<std::uint32_t,
                       std::set<rafko_net::Neuron_group_features>>
        map_layers_to_features;
    for (std::uint32_t layer_index = 0u; layer_index < net_structure.size();
         ++layer_index) {
      for (std::uint32_t tries = 0; tries < 5u; ++tries) {
        rafko_net::Neuron_group_features random_feature =
            static_cast<rafko_net::Neuron_group_features>(
                rand() % rafko_net::Neuron_group_features_ARRAYSIZE);
        while (!Neuron_group_features_IsValid(random_feature) ||
        ...
        if (feature_set_in_layer_it != map_layers_to_features.end() &&
    ...
    for (const auto &[layer_index, features_for_layer] :
         map_layers_to_features) {
      REQUIRE(std::find_if(/* network feature groups should contain
                              features_for_layer */
                           network.neuron_group_features().begin(),
                           network.neuron_group_features().end(),
                           [&features_for_layer, &layer_starts, net_structure,
                            layer_index](
                               const rafko_net::FeatureGroup &feature_group) {
                             if (0 < features_for_layer.count(
                                         feature_group.feature())) {
                               return (/* If the feature group is found in the
                                          original features for the layers.. */
                                       (feature_group.relevant_neurons_size() ==
                                        1) /* the start and size value of it
                                              should match the layers' */
                                       && (feature_group.relevant_neurons(0)
                                               .starts() ==
                                           static_cast<std::int32_t>(
                                               layer_starts[layer_index])) &&
                                       (feature_group.relevant_neurons(0)
                                            .interval_size() ==
                                        net_structure[layer_index]));
                             }
                             return false;
                           }) != network.neuron_group_features().end());
    }
...
TEST_CASE("Testing builder for setting Neuron input functions manually",
  ...
  for (std::uint32_t variant = 0u; variant < 10u; ++variant) {
    ...
    std::vector<
        std::tuple<std::uint32_t, std::uint32_t, rafko_net::Input_functions>>
        set_neuron_input_functions;
    for (std::uint32_t layer_index = 0u; layer_index < net_structure.size();
         ++layer_index) {
      for (std::uint32_t tries = 0; tries < 5u; ++tries) {
        std::uint32_t layer_neuron_index =
            rand() % (net_structure[layer_index]);
        std::tuple<std::uint32_t, std::uint32_t, rafko_net::Input_functions>
            new_item = std::make_tuple(
                layer_index, layer_neuron_index,
                rafko_net::InputFunction::next(
                    rafko_net::InputFunction::all_input_functions));
        if (/* only add the the reference vector if its not present yet in the
               same layer_index/neuron_index */
            std::find_if(
                set_neuron_input_functions.begin(),
                set_neuron_input_functions.end(),
                [&new_item](const std::tuple<std::uint32_t, std::uint32_t,
                                             rafko_net::Input_functions>
                                &current_item) {
                  return (
                      (std::get<0>(new_item) == std::get<0>(current_item)) &&
                      (std::get<1>(new_item) == std::get<1>(current_item)));
                }) == set_neuron_input_functions.end()) {
          builder.set_neuron_input_function(layer_index, layer_neuron_index,
                                            std::get<2>(new_item));
    ...
    for (const std::tuple<std::uint32_t, std::uint32_t,
                          rafko_net::Input_functions> &element :
         set_neuron_input_functions) {
      ...
      std::uint32_t neuron_index =
          layer_starts[std::get<0>(element)] + std::get<1>(element);

      REQUIRE(network.neuron_array(neuron_index).input_function() ==
              std::get<2>(element));
    }
  } /*for(10 variants)*/
}

TEST_CASE("Testing builder for setting Neuron transfer functions manually",
  ...
  for (std::uint32_t variant = 0u; variant < 10u; ++variant) {
    ...
    std::vector<
        std::tuple<std::uint32_t, std::uint32_t, rafko_net::Transfer_functions>>
        set_neuron_transfer_functions;
    for (std::uint32_t layer_index = 0u; layer_index < net_structure.size();
         ++layer_index) {
      for (std::uint32_t tries = 0; tries < 5u; ++tries) {
        std::uint32_t layer_neuron_index =
            rand() % (net_structure[layer_index]);
        std::tuple<std::uint32_t, std::uint32_t, rafko_net::Transfer_functions>
            new_item = std::make_tuple(layer_index, layer_neuron_index,
                                       rafko_net::TransferFunction::next());
        if (/* only add the the reference vector if its not present yet in the
               same layer_index/neuron_index */
            std::find_if(
                set_neuron_transfer_functions.begin(),
                set_neuron_transfer_functions.end(),
                [&new_item](const std::tuple<std::uint32_t, std::uint32_t,
                                             rafko_net::Transfer_functions>
                                &current_item) {
                  return (
                      (std::get<0>(new_item) == std::get<0>(current_item)) &&
                      (std::get<1>(new_item) == std::get<1>(current_item)));
                }) == set_neuron_transfer_functions.end()) {
          builder.set_neuron_transfer_function(layer_index, layer_neuron_index,
                                               std::get<2>(new_item));
    ...
    for (const auto &[layer_index, layer_neuron_index, transfer_function] :
         set_neuron_transfer_functions) {
      REQUIRE(layer_index < net_structure.size());
      REQUIRE(layer_neuron_index < net_structure[layer_index]);
      std::uint32_t neuron_index =
          layer_starts[layer_index] + layer_neuron_index;
      REQUIRE(network.neuron_array(neuron_index).transfer_function() ==
              transfer_function);
    }
  } /*for(10 variants)*/
}

TEST_CASE("Testing whether network builder throws an excpetion when "
          "conflicting transfer function parameters are set",
          "[build][transfer-function]") {
  google::protobuf::Arena arena;
  ...
  for (std::uint32_t trf = rafko_net::Transfer_functions_MIN + 1;
       trf < rafko_net::Transfer_functions_MAX; ++trf) {
    if (!rafko_net::Transfer_functions_IsValid(trf))
      continue;
    rafko_net::Transfer_functions transfer_function =
        static_cast<rafko_net::Transfer_functions>(trf);
    REQUIRE_NOTHROW(rafko_net::RafkoNetBuilder(settings)
                        .input_size(1u)
...
TEST_CASE("Testing builder for setting Neuron spike functions manually",
          "[build][spike-function]") {
  google::protobuf::Arena arena;
  rafko_mainframe::RafkoSettings settings =
      rafko_mainframe::RafkoSettings().set_arena_ptr(&arena);
  for (std::uint32_t variant = 0u; variant < 10u; ++variant) {
    ...
    std::vector<
        std::tuple<std::uint32_t, std::uint32_t, rafko_net::Spike_functions>>
        set_neuron_spike_functions;
    for (std::uint32_t layer_index = 0u; layer_index < net_structure.size();
         ++layer_index) {
      for (std::uint32_t tries = 0; tries < 5u; ++tries) {
        std::uint32_t layer_neuron_index =
        ...
        std::tuple<std::uint32_t, std::uint32_t, rafko_net::Spike_functions>
            new_item = std::make_tuple(
                layer_index, layer_neuron_index,
                rafko_net::SpikeFunction::next(
                    rafko_net::SpikeFunction::all_spike_functions));
        if (/* only add the the reference vector if its not present yet in the
               same layer_index/neuron_index */
            std::find_if(
                set_neuron_spike_functions.begin(),
                set_neuron_spike_functions.end(),
                [&new_item](const std::tuple<std::uint32_t, std::uint32_t,
                                             rafko_net::Spike_functions>
                                &current_item) {
                  return (
                      (std::get<0>(new_item) == std::get<0>(current_item)) &&
                      (std::get<1>(new_item) == std::get<1>(current_item)));
                }) == set_neuron_spike_functions.end()) {
          builder.set_neuron_spike_function(layer_index, layer_neuron_index,
                                            std::get<2>(new_item));
    ...
    for (const std::tuple<std::uint32_t, std::uint32_t,
                          rafko_net::Spike_functions> &element :
         set_neuron_spike_functions) {
      REQUIRE(std::get<0>(element) < layer_starts.size());
      REQUIRE(std::get<1>(element) < net_structure[std::get<0>(element)]);
      std::uint32_t neuron_index =
      ...
      REQUIRE(network.neuron_array(neuron_index).spike_function() ==
              std::get<2>(element));
    }
  } /*for(10 variants)*/
}

TEST_CASE("Testing Convolutional input added to layers",
  ...
  rafko_net::RafkoNet &network =
      *rafko_net::RafkoNetBuilder(*settings)
           ...
           /*Note: Input or output dimensions need not be added, but compared to
              layer sizes when building */
           .kernel_size(2, 2)
           .kernel_stride(2, 2)
           .input_padding(0, 0)
           .input_size(8, 8)
