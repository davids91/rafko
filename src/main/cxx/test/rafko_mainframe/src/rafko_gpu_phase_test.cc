...
TEST_CASE("Testing if the simplest OpenCL Implementation is working as expected with std::vector input", "[GPU][Phase][simple]" ) {
  constexpr const std::size_t element_count = 10;
  std::shared_ptr<EchoStrategy> strategy = std::make_unique<EchoStrategy>(element_count);
  ...
  REQUIRE_THAT( input_vector, Catch::Matchers::Approx(result_vector).margin(0.0000000000001) );
}

TEST_CASE("Testing if the simplest OpenCL Implementation is working as expected with cl::Buffer input", "[GPU][Phase][simple]" ) {
  constexpr const std::size_t element_count = 10;
  std::shared_ptr<EchoStrategy> strategy = std::make_unique<EchoStrategy>(element_count);
  rafko_mainframe::RafkoOCLFactory cl_factory = (
    rafko_mainframe::RafkoOCLFactory().select_platform().select_device()
  ...
  std::vector<double> input_vector(element_count, 666);
  cl::Buffer input_buffer(context, CL_MEM_READ_WRITE, sizeof(double) * element_count);
  std::vector<double> result_vector(element_count);

  cl_int return_value = queue.enqueueWriteBuffer(
    input_buffer, CL_TRUE/* blocking */,
    0/*offset*/, sizeof(double) * element_count/*size*/,
    input_vector.data()
  );
...
TEST_CASE("Testing if the simplest OpenCL Implementation is working as expected with setting the input cl::Buffer of the phase", "[GPU][Phase][simple]" ) {
  constexpr const std::size_t element_count = 10;
  std::shared_ptr<EchoStrategy> strategy = std::make_unique<EchoStrategy>(element_count);
  rafko_mainframe::RafkoOCLFactory cl_factory = (
    rafko_mainframe::RafkoOCLFactory().select_platform().select_device()
  ...
  cl_int return_value = queue.enqueueWriteBuffer(
    test_phase->get_input_buffer(), CL_TRUE/* blocking */, 
    0/*offset*/, sizeof(double) * element_count/*size*/,
    input_vector.data()
  );
  REQUIRE( return_value == CL_SUCCESS );
