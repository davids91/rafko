 ...
 * */
void check_data_match(std::vector<double>& sample_data, std::vector<double>& ringbuffer_data){
...
TEST_CASE("Testing Data Ringbuffer implementation", "[data-handling]"){
  ...
  rafko_utilities::DataRingbuffer<> buffer(buffer_number,[buffer_size](std::vector<double>& element){
  ...
  /* resetting buffers */
  buffer.reset();
  for(std::uint32_t past_index = 0u; past_index < buffer_number; ++past_index){
...
TEST_CASE("Testing if ringbuffer past indexing logic is as expected", "[data-handling]"){
  ...
  std::uint32_t buffer_size = 30;
  rafko_utilities::DataRingbuffer<> buffer(sequence_number,[buffer_size](std::vector<double>& element){
    element = std::vector<double>(buffer_size, 0.0);
  });
  ...
  for(std::uint32_t i = 0; i < sequence_number; i++)
    std::cout << "[" << i << "]-";
