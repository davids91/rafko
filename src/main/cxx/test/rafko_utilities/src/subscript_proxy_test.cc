...
TEST_CASE("Testing if Subscript Proxy works as expected",
          "[data-handling][proxy]") {
  ...
  while (0.0 < tmp_value) {
    ...
    while (0 != dictionary.count(index))
      index = rand() % base_vector.size();
    dictionary.insert({index, rand() % base_vector.size()});
    tmp_value -= static_cast<double>(rand() % 10) / 2.0;
    if (dictionary.size() == base_vector.size())
      break;
  }
