...
TEST_CASE("Testing if ThreadGroups can be combined", "[thread-group][multi-thread]"){
  ...
  for(std::uint32_t variant = 0; variant < 10u; ++variant){
    ...
    double check = 0;
    double check2 = 0;
    std::vector<std::vector<double>> test_buffer(rand()%5 + 1, std::vector<double>(rand()%5 + 1));
    std::for_each(test_buffer.begin(),test_buffer.end(),[&](std::vector<double>& vec){
    ...
    result = (0.0);
    std::function<void(std::uint32_t)> outer_sum = [&](std::uint32_t thread_index){
      ...
      if(outer_start < test_buffer.size()){ /* More threads could be available, than needed */
        ...
        for(size_t i = 0; i < outer_length; ++i){
          ...
          std::function<void(std::uint32_t)> inner_sum = [&](std::uint32_t inner_thread_index){
...
TEST_CASE("Testing if ThreadGroups can be combined in a thread-safe manner", "[thread-group][multi-thread]"){
  const std::uint32_t number_of_threads = 5;
  double expected = (0.0);
  double result = (0.0);
  std::mutex result_mutex;
