 ...
 * */
TEST_CASE("Error function mean squared error test",
  ...
  for (std::uint16_t sample_iterator = 0; sample_iterator < dataset_size;
    ...
    for (std::uint16_t feature_iterator = 0; feature_iterator < feature_size;
         ++feature_iterator) {
      dataset[sample_iterator][feature_iterator] =
          static_cast<double>(rand() % dataset_size);
      if (0 ==
          (rand() % 2)) /* For every data feature and every sample, the distance
                           should be equal to the correspoding datapoints */
        featureset[sample_iterator][feature_iterator] =
            dataset[sample_iterator][feature_iterator] + distance;
      else
        featureset[sample_iterator][feature_iterator] =
            dataset[sample_iterator][feature_iterator] - distance;
    }
  ...
  SECTION("Testing one feature distance (should be (0.5 * (distance)^2) )") {
    for (std::uint16_t sample_iterator = 0; sample_iterator < dataset_size;
         ++sample_iterator) {
      REQUIRE(cost.get_feature_error(dataset[sample_iterator],
                                     featureset[sample_iterator],
                                     dataset_size) ==
              Catch::Approx((0.5 * feature_size * pow(distance, 2)) /
  ...
  SECTION("Test if the whole dataset can be processed in one function call") {
    ...
    for (const double label_error : label_errors) {
      CHECK(Catch::Approx(label_error).epsilon((0.00000000000001)) ==
            (0.5 * feature_size * pow(distance, 2)) /
                static_cast<double>(dataset_size));
    }
  }

  SECTION("Testing if the correct cost is produced when calling the function "
    ...
    cost.get_feature_errors(dataset, featureset, label_errors, 0, 0,
                            label_errors.size(), 0, dataset_size);
    for (const double label_error : label_errors)
      error_sum_reference += label_error;

    threads.start_and_block(
        [&dataset, &featureset, &cost, &error_sum](std::uint32_t thread_index) {
          double current_error = error_sum;
          const double addition = cost.get_feature_error(
              dataset[thread_index], featureset[thread_index], dataset_size);
          while (!error_sum.compare_exchange_weak(current_error,
                                                  (current_error + addition)))
            current_error = error_sum;
        });
    REQUIRE(Catch::Approx(error_sum).epsilon((0.00000000000001)) ==
            error_sum_reference);
  }
}
