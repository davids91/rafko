...
/**
 * @brief     A collcetion of characters to help level generation for
 * @ConsoleJumper
 */
...
/**
 * @brief     An environment where the player (`o`) tries to jump to the end of
 * the console, with boosters(`>`), setbacks(`<`) their extenders (`=`) its path
 * besides non-effect characters (`#`) and teleportation pads(`^`).
 *            Teleportation pads trasnfers the player to the last of it in the
 * console line. The player has a limited vision of its surroundings.
...
class ConsoleJumper : public rafko_gym::RafQEnvironment {
  ...
  inline static CharacterCollection s_allCharacters =
      CharacterCollection("####=<>");
  inline static CharacterCollection s_extensions = CharacterCollection("=");
  inline static CharacterCollection s_extensionStoppers =
      CharacterCollection("<>");

  static std::vector<char>
  generate_level(std::uint32_t width, std::uint32_t sight,
                 std::uint32_t &last_teleport_position) {
    REQUIRE(1 < width);
    ...
    for (std::uint32_t char_index = 1; char_index < width; ++char_index) {
      ...
      if (0 < current_extender_count) {
        ...
        if (s_extensionStoppers.contains(prev))
          ...
          c = s_allCharacters.next();
      }
      if (c == '=')
        ++current_extender_count;
      if (c == '#')
        ++current_path_count;
      else
        current_path_count = 0;
...
public:
  ConsoleJumper(std::uint32_t width = 80, std::uint32_t sight = 5)
      : rafko_gym::RafQEnvironment((1u + sight), 1, {100, 100}, {0, 7}),
        ...
        m_actualState(1u + m_sight), m_tmpState(1u + m_sight) {
    using LevelView =
        rafko_utilities::ConstVectorSubrange<std::vector<char>::const_iterator>;

    for (std::uint32_t pos = 0; pos < m_level.size(); ++pos) {
      const std::uint32_t start_index = pos - std::min(m_sight, pos);
      const std::uint32_t state_count = std::min(
          static_cast<std::size_t>(m_sight), (m_level.size() - start_index));
      ...
      std::transform(m_statesBuffer[pos].begin(), m_statesBuffer[pos].end(),
                     m_statesBuffer[pos].begin(),
                     [&buffer_index, &view_index, &level_sight, start_in_buffer,
                      end_in_buffer](const double &) {
                       if ((buffer_index >= start_in_buffer) &&
                           (buffer_index < end_in_buffer) &&
  ...
  void print() {
    ...
    std::cout << "(" << m_actualState[0]
              << ")  "; /* the first element of the state is energy */
  }

  std::size_t state_size() { return m_actualState.size(); }

  void reset() override {
    m_pos = 0;
    m_actualState[0] = m_level.size() / 2.0;
    std::copy(m_statesBuffer[m_pos].begin(), m_statesBuffer[m_pos].end(),
              m_actualState.begin() + 1u);
  }

  StateTransition current_state() const override {
    if (m_pos < 0 || static_cast<std::int32_t>(m_level.size()) < m_pos) {
      return {{}, 0.0, true, AnyData(new std::int32_t(m_pos), [](void *ptr) {
                delete static_cast<int *>(ptr);
              })};
    }
    return {m_actualState,
            get_q_value(static_cast<double>(m_pos), m_actualState[0]), false,
            AnyData(new std::int32_t(m_pos),
                    [](void *ptr) { delete static_cast<int *>(ptr); })};
  }

  StateTransition next(FeatureView action) override {
    if (m_sight < std::abs(action[0]) ||
        (m_actualState[0] <=
         0.0)) /* if player tries to jump too much, or lost all energy it dies*/
      return {{},
              static_cast<double>(m_pos),
              true,
              AnyData(new std::int32_t(m_pos),
                      [](void *ptr) { delete static_cast<int *>(ptr); })};
    ...
    if ((m_pos < 0 || static_cast<std::int32_t>(m_level.size()) <= m_pos) ||
      ...
      return {
          {}, q_value, true, AnyData(new std::int32_t(m_pos), [](void *ptr) {
            delete static_cast<int *>(ptr);
          })};
    }
    std::copy(m_statesBuffer[m_pos].begin(), m_statesBuffer[m_pos].end(),
              m_actualState.begin() + 1);
  ...
  StateTransition next(FeatureView state, FeatureView action,
    ...
    if (m_sight < std::abs(action[0]) ||
        (m_tmpState[0] <=
         0.0)) /* if player tries to jump too much, or lost all energy it dies*/
      return {{},
              (static_cast<double>(pos) + m_tmpState[0]),
              true,
              AnyData(new std::int32_t(pos),
                      [](void *ptr) { delete static_cast<int *>(ptr); })};

    auto state_it = std::find_if(
        m_statesBuffer.begin(), m_statesBuffer.end(),
        [&state](const FeatureVector &stored_state) {
          ...
          return std::all_of(stored_state.begin(), stored_state.end(),
                             [&state, &i](const double &element) {
                               return element == state[i++];
                             });
        });

    if (state_it == m_statesBuffer.end())
      return {{}, 0.0, true, AnyData(new std::int32_t(pos), [](void *ptr) {
                delete static_cast<int *>(ptr);
              })};

    /* generated state needs to match the provided state, to check if the
     * user_data is transferred correctly through the trainer! */
    REQUIRE_THAT(
        *state_it,
        Catch::Matchers::Approx(m_statesBuffer[pos]).margin(0.0000000000001));
    ...
    if ((result_index < static_cast<std::int32_t>(m_level.size())) &&
      ...
      std::copy(m_statesBuffer[result_index].begin(),
                m_statesBuffer[result_index].end(), m_tmpState.begin() + 1);
      return {{m_tmpState},
              q_value,
              false,
              AnyData(new std::int32_t(pos),
      ...
      return {{}, q_value, true, AnyData(new std::int32_t(pos), [](void *ptr) {
                delete static_cast<int *>(ptr);
              })};
    }
  }
...
private:
  ...
  static void process(const std::vector<char> &level, std::int32_t &pos,
                      std::uint32_t last_teleport_position) {
    if (pos < 0 || static_cast<std::int32_t>(level.size()) <= pos)
      return;
    if ('^' == level[pos]) {
...
TEST_CASE("Testing if RafQTrainer works as expected with a simple board game "
          "simulation",
          "[optimize][QLearning][!benchmark]") {
  ...
  std::shared_ptr<rafko_mainframe::RafkoSettings> settings = std::make_shared<
      ...
      rafko_mainframe::RafkoSettings()
          ...
          .set_minibatch_size(policy_q_set_size / 10)
          ...
          .set_max_processing_threads(3));

  rafko_net::RafkoNet &network =
      *rafko_net::RafkoNetBuilder(*settings)
           .input_size(test_game->state_size())
           .expected_input_range(
               test_game->state_properties().m_standardDeviation)
           ...
           .allowed_transfer_functions_by_layer(
               {{rafko_net::transfer_function_selu},
                ...
                {rafko_net::transfer_function_selu}})
           .create_layers(
               {5, 5, (policy_action_count * (1 + policy_action_size))});
  ...
  std::shared_ptr<rafko_net::SolutionSolver> reference_solver =
      solverFactory.build();
  rafko_gym::RafQTrainer trainer(network, policy_action_count,
                                 policy_q_set_size, test_game, objective,
                                 settings);
  trainer.set_weight_updater(rafko_gym::weight_updater_amsgrad);
  std::uint32_t iteration = 1;
  while (true) {
    double exploration_ratio =
        (1.0 - (static_cast<double>(std::min(500u, iteration)) / 501));
    bool terminal = false;
    ...
    while (!terminal && steps < 200) {
      std::cout << "\r"
                ...
                << ((exploration_ratio > 0.8)
                        ? (0.0)
                        : ((1.0 - exploration_ratio) * 1000))
                << "    "
                << "; iter: " << iteration << "; expl: " << exploration_ratio
                << "; qSet size: " << trainer.q_set_size() << "; err: "
                << trainer.full_evaluation(true /*force_gpu_upload*/) << "; ";
      test_game->print();
      ...
      auto policy_action = reference_solver->solve(
          test_game->current_state().m_resultState.value(),
          true /*reset_neuron_data*/
      );
    ...
    trainer.iterate(
        200 /*max_discovery_length*/, exploration_ratio,
        (exploration_ratio > 0.8)
            ? (0)
            : ((1.0 - exploration_ratio) * 1000) /*q_set_training_epochs*/,
        [](double progress, std::uint32_t) {
          std::cout << "\r progress: " << (progress * 100) << "%   "
                    << std::flush;
        });
    ++iteration;
  }
