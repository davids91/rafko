...
namespace {
  rafko_gym::RafQEnvironment::FeatureVector action_slot(const rafko_gym::RafQEnvironment::FeatureVector& action, double q_value){
...
public:
  ...
  StateTransition current_state() const override{
    return {
      m_state, s_stateQValues.at(m_state[0]), s_stateTerminalValues.at(m_state[0]),
      AnyData(new double(m_state[0]), [](void* ptr){ delete static_cast<double*>(ptr); })
    };
  }
  ...
  StateTransition next(FeatureView state, FeatureView action, const AnyData& user_data = {}) const override{
    ...
    if(result_state == s_stateTransitions.end()){
      ...
      return {
        ...
        AnyData(new double((*state_iterator)[0]), [](void* ptr){ delete static_cast<double*>(ptr); })
      };
    }
  }
private: 
...
TEST_CASE("Testing if RafQSet element insertion works as expected", "[QSet][QLearning]") {
  ...
  SECTION("Adding new actions for the same state"){
    ...
    new (&next_state)TestEnvironment::StateTransition(environment.next(std::vector<double>{1.0}, std::vector<double>{2.0}));
    REQUIRE(static_cast<bool>(next_state.m_userData));
    q_set.incorporate({ {1.0} }, { action_slot({2.0}, next_state.m_resultQValue) }, std::move(user_data));
    ...
    user_data.push_back(std::move(next_state.m_userData));
    new (&next_state)TestEnvironment::StateTransition(environment.next(std::vector<double>{1.0}, std::vector<double>{4.0}));
    REQUIRE(static_cast<bool>(next_state.m_userData));
    q_set.incorporate({ {1.0} }, { action_slot( {4.0}, next_state.m_resultQValue) }, std::move(user_data2));
    REQUIRE(1 == q_set.get_number_of_sequences());
  ...
  SECTION("Checking if a worse initial action, which would lead to a better state gets stored"){
    ...
    q_set.incorporate(
      { {1.0} }, { action_slot({4.0}, environment.next(std::vector<double>{1.0}, std::vector<double>{4.0}).m_resultQValue) }
    );
    REQUIRE(1 == q_set.get_number_of_sequences());
    ...
    q_set.incorporate(
      { {3.0} }, { action_slot({5.0}, environment.next(std::vector<double>{3.0}, std::vector<double>{5.0}).m_resultQValue) }
    );
  ...
  SECTION("Checking if the QSet keeps it's maximum size, and the worse states always get deleted"){
    ...
    for(std::uint32_t element_index = 1; element_index <= elements_to_upload; ++element_index){
      q_set.incorporate(
        ...
        {
          action_slot({4.0}/*action*/, static_cast<double>(element_index + element_index)),
          action_slot({3.0}/*action*/, static_cast<double>(element_index + 2 * element_index)),
          action_slot({2.0}/*action*/, static_cast<double>(element_index + 3 * element_index)),
          action_slot({1.0}/*action*/, static_cast<double>(element_index + 4 * element_index))
        }
      );   
    }
    ...
    for(double state_index = 0; state_index < TestEnvironment::states_count(); state_index += 1.0){
      ...
      for(double action_index = 0; action_index < TestEnvironment::states_count(); action_index += 1.0){
        ...
        if(state_transition.m_resultState.has_value()){
          actions_for_state.push_back(action_slot(
