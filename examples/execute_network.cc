...
int main(int argc, char* argv[]){
  ...
  std::unique_ptr<rafko_net::SolutionSolver> solver_from_factory = solverFactory.build();
   ...
   */
  std::unique_ptr<rafko_net::SolutionSolver> tinkered_network_solver = rafko_net::SolutionSolver::Factory(*network, settings).build();

  solver_from_factory->set_eval_mode(true);
  ...
  // std::unique_ptr<rafko_net::SolutionSolver> another_solver_from_the_same_factory = solverFactory.build();
  //
  ...
  //  */
  // std::unique_ptr<rafko_net::SolutionSolver> updated_solver = solverFactory.build(true/*rebuild_solution*/);
  ...
  //
  // /*!Note: To handle Solutions more efficiently the factory can be used to replace the last created Solution.
  //  * It however only replaces the lat built Solution, so Solver built based on previous Solutions are not updated.
  //  */
  // //TODO: present SWAP
  //
  // //TODO: Context solve
