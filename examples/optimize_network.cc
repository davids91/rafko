...
int main(){
  ...
  /*!Note: Optimizer works based on gradients, but to see the progress of training, and to enable more advanced features
   ...
   * the error rate is below @low_error for at least @iteration_optimum_delta iterations
   */
  ...
  std::chrono::steady_clock::time_point start; /* To display runtime statistics, the optimization is going to be timed */
  std::uint32_t avg_duration;

  /*!Note: Calcualte the width of the used console, this helps clear the given row, so data from one iteration
   * can overwrite the current row unless it is a new error minimum. Clearing the row beforehand makes sure that
   * the numbers contain no thrash data from a previous iteration (displayed on the same row)
   */
  ...
  while(!optimizer->stop_triggered()){ /* If Early stopping or zero-error condition is set, the training quits */
  ...
  << " iterations!(average runtime: "<< avg_duration << " ms)   " << std::endl;
